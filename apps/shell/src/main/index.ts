...
function registerHomeIpc(): void {
  ...
  ipcMain.handle(HOME_CHANNELS.exportFile, async (_event, path: unknown) => {
    if (typeof path !== 'string' || !existsSync(path)) {
      return { ok: false, error: 'File not found' }
    }
    if (!shellWindow) return { ok: false, error: 'No window' }
    const picked = await showSaveDialogWithMemory(dialog, shellWindow, {
      defaultPath: path,
    })
    if (picked.canceled || !picked.filePath) return { ok: true }
    if (picked.filePath === path) return { ok: true }
    try {
      copyFileSync(path, picked.filePath)
      return { ok: true }
    } catch (error) {
