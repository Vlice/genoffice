...
export function SlideCanvas({
  ...
  useEffect(() => {
    ...
    if (!tr || !layer) return
    // Text edit overlay draws the frame; keeping Transformer attached stacked a second
    // identical box (empty Insert>Text Box is short overlay vs 60px handles).
    if (editingText && !editingText.cell) {
      tr.nodes([])
      tr.getLayer()?.batchDraw()
      return
    }
    ...
    tr.nodes(nodes)
    tr.getLayer()?.batchDraw()
  }, [selectedIds, slide, enteredGroupId, editingText])
