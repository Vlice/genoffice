...
export function SlideShowView({
  ...
  const transReadyRef = useRef(false)
  ...
  useEffect(() => {
    let cancelled = false
    transReadyRef.current = false
    void Promise.all(slides.map((_, i) => window.slidesApi.getTransition(i))).then((kinds) => {
      if (!cancelled) {
        transRef.current = kinds
        transReadyRef.current = true
      }
    })
  ...
  const goTo = useCallback(
    (nextPos: number, animate: boolean) => {
      ...
      const applyKind = (raw: TransitionKind) => {
        let kind = raw
      ...
      const cached = transRef.current[target]
      if (transReadyRef.current || cached != null) {
        applyKind(cached ?? 'none')
        return
      }
      // Prefetch still in flight — resolve this page before turning so the first
      // click after "应用到全部" doesn't play as a no-op.
      void window.slidesApi.getTransition(target).then((k) => {
        const next = [...transRef.current]
        next[target] = k
  ...
  const followLink = useCallback(
    (target: LinkTargetOp) => {
      ...
      const href = target.kind === 'url' ? target.url : ''
