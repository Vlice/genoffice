...
export interface LinkDialogState {
  ...
  run?: boolean
  /** Prefilled “text to display”; empty keeps the current selection/run text */
  displayText?: string
}
...
export interface ActionCtx {
  ...
  /** True while a manual/auto save is in flight — late setDirty(true) must not flip the ribbon. */
  savingRef: { current: boolean }
  /** User edited after save started; re-dirty when the in-flight save finishes. */
  unsavedDuringSaveRef: { current: boolean }
  /** Immediate unsaved (ribbon). During save, records a follow-up dirty instead of flipping UI. */
  markUnsaved: () => void
