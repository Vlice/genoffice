...
export async function openLinkDialog(ctx: ActionCtx): Promise<void> {
  ...
  if (inTextEdit) {
    ...
    ctx.setLinkDialog({
      sourceId: null,
      run: true,
      initial: selectionLink(),
      displayText: selectionDisplayText(),
    })
    return
  }
  if (ctx.selectedIds.length !== 1) return
  const sourceId = ctx.selectedIds[0]!
  // Text boxes: enter the overlay so the URL attaches to glyphs (docs-like), not only the
  // shape. Pictures / shapes without an editor still use element-level hlinkClick.
  const node = ctx.findNodeCtx(sourceId)?.node as { text?: unknown } | undefined
  if (node?.text) {
    ctx.startEdit(sourceId)
    const initial = await window.slidesApi.getLink(ctx.current, sourceId)
    ctx.setLinkDialog({ sourceId, run: true, initial, displayText: '' })
    return
  }
  const initial = await window.slidesApi.getLink(ctx.current, sourceId)
...
export async function applyLink(
  ...
  if (ctx.linkDialog.run) {
    const ok = applySelectionLink(target, displayText)
