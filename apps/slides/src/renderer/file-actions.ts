...
export async function save(ctx: ActionCtx, quiet = false): Promise<boolean> {
  // Clear ribbon status immediately on ⌘S — don't wait for overlay commit / pptx / cloud.
  ctx.savingRef.current = true
  ctx.unsavedDuringSaveRef.current = false
  ctx.setDirty(false)
  let ok = false
  try {
    ...
    const pending = ctx.unsavedDuringSaveRef.current
    ctx.savingRef.current = false
    ctx.setDirty(!ok || pending)
  }
}
...
export async function saveAs(ctx: ActionCtx): Promise<void> {
  ...
  try {
    ...
    if (r.ok) {
      ...
      ctx.setDirty(ctx.unsavedDuringSaveRef.current)
      const saved = t('appStatusSavedAs')
      ctx.setStatus(saved)
      showToast(saved)
    } else if (r.error) {
      ctx.unsavedDuringSaveRef.current = true
