...
/** MoreAI's platform default. That choice must not receive screenshots. */
const MOREAI_SYSTEM_MODEL_ID = '__system__'
...
export function settingsSupportVision(
  settings: Pick<AiSettings, 'provider' | 'providers'> & { moreaiModelId?: string },
): boolean {
  try {
    // MoreAI always reports provider "custom", whose metadata claims vision.
    // The system default is not a vision model: skip screenshots so a clean
    // geometry audit can finish without a model call.
    if (settings.moreaiModelId === MOREAI_SYSTEM_MODEL_ID) return false
