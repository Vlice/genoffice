...
import { isSlideImageRef } from '../shared/image-refs'
...
export function parsePageSpec(
  ...
  for (const [i, rawEl] of rawEls.entries()) {
    ...
    if (type === 'image') {
      ...
      if (!isSlideImageRef(url)) {
        warnings.push(`element ${i}: image url must be http(s), attachment:N, or data:image, dropped`)
        continue
      }
...
async function growTextBoxesToContent(
  ...
  if (separateOverlappingTextBoxes(slide, opened.deck.size.cy)) changed = true
  return changed ? savePptx(opened) : null
}

/** After undersized titles grow downward they collide with the next text box. Push the
 * lower box down (8px gap) instead of leaving two bodies painted on top of each other. */
function separateOverlappingTextBoxes(
  slide: {
    elements: Array<{ type?: string; transform: TextElement['transform']; dirtyTransform?: boolean }>
  },
  slideCy: number,
): boolean {
  const gap = Math.round(8 * EMU_PER_PX_96)
  const texts = slide.elements.filter((e) => e.type === 'text') as Array<{
    transform: TextElement['transform']
    dirtyTransform?: boolean
  }>
  texts.sort((a, b) => a.transform.offset.y - b.transform.offset.y)
  let changed = false
  for (let pass = 0; pass < 8; pass++) {
    let moved = false
    for (let i = 0; i < texts.length; i++) {
      const a = texts[i]!.transform.offset
      for (let j = i + 1; j < texts.length; j++) {
        const b = texts[j]!.transform.offset
        if (a.x >= b.x + b.cx || a.x + a.cx <= b.x) continue
        const push = a.y + a.cy + gap - b.y
        if (push <= 0.5 * EMU_PER_PX_96) continue
        b.y += Math.round(push)
        if (b.y + b.cy > slideCy) b.y = Math.max(0, slideCy - b.cy)
        texts[j]!.dirtyTransform = true
        moved = true
        changed = true
      }
    }
