...
import { dataImageBytes } from '../shared/image-refs'
...
function applyAutofitResize(
  ...
  if (!el?.text || el.placeholder) return rendered
  if (el.text.autofit === 'shrink') return rendered
  if (el.text.autofit !== 'resize' && !el.txBox) return rendered
...
export function registerSlidesIpc(): void {
  ...
  ipcMain.handle('slides:edit-transform', (e, op: EditTransformOp) => {
    ...
    const rendered = rebuildSlide(session, op.slideIndex)
    return applyAutofitResize(session, op.slideIndex, childId, rendered)
  })
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        const started = Date.now()
        const supplied = op.imageBytes ?? {}
        const { bytes, imageFailures } = await buildPagePptx(parsed.spec, {
          fontMetrics: getFontMetrics(),
          fetchImage: async (url) => {
            const mapped = supplied[url]
            if (mapped?.base64) {
              return { bytes: Uint8Array.from(Buffer.from(mapped.base64, 'base64')), ext: mapped.ext }
            }
            const data = dataImageBytes(url)
            if (data) return data
