...
describe('regenerate_slide', () => {
  ...
  it('embeds unused user photos via local gen even when cloud is on', async () => {
    const generatePageLocal = vi.fn(async (args: { images: string[] }) => {
      expect(args.images).toEqual(['attachment:0'])
      return { ok: true, marker: 'localpptx:/tmp/p.pptx' }
    })
    const generatePageCloud = vi.fn(async () => ({ ok: true, marker: 'cloudpptx:/tmp/c.pptx' }))
    const skill = createSlidesSkill(
      mkAccess([page], {
        regenerateSlide: vi.fn(async () => ({ ok: true })),
        generatePageLocal,
    ...
    const r = await skill.executeTool!(
      call('regenerate_slide', { slideIndex: 0, brief: 'put the uploaded photo on the cover' }),
    )
    expect(r.isError).toBeUndefined()
    expect(generatePageLocal).toHaveBeenCalledOnce()
    expect(generatePageCloud).not.toHaveBeenCalled()
  })
