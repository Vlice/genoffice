...
import { CanvasMetrics, HeuristicMetrics } from '@genoffice/pptx-render'
...
describe('parsePageSpec', () => {
  ...
  it('keeps attachment:N and data:image urls', () => {
    const r = parsePageSpec(
      JSON.stringify({
        elements: [
          { type: 'image', url: 'attachment:0', x: 0, y: 0, w: 120, h: 80 },
          {
            type: 'image',
    ...
    expect(imgs.map((e) => (e as { url: string }).url)).toEqual([
      'attachment:0',
...
describe('page-gen font metrics constructors', () => {
  it('CanvasMetrics and HeuristicMetrics are constructable', () => {
    expect(typeof HeuristicMetrics).toBe('function')
    expect(typeof CanvasMetrics).toBe('function')
    expect(new HeuristicMetrics()).toBeInstanceOf(HeuristicMetrics)
    expect(new CanvasMetrics()).toBeInstanceOf(CanvasMetrics)
  })
})
...
describe('buildPagePptx', () => {
  ...
  it('embeds attachment:N when fetchImage resolves the ref', async () => {
    const spec: PageSpec = {
      elements: [
        { type: 'image', url: 'attachment:0', x: 0, y: 0, w: 200, h: 200 },
        { type: 'text', x: 20, y: 20, w: 100, h: 40, paragraphs: [{ runs: [{ text: 'hi' }] }] },
      ],
    }
    const { bytes, imageFailures } = await buildPagePptx(spec, {
      fetchImage: async (url) => (url === 'attachment:0' ? { bytes: PNG_1PX, ext: 'png' } : null),
    })
    expect(imageFailures).toEqual([])
    const opened = await openPptx(bytes)
    const pics = opened.deck.slides[0]!.elements.filter(
...
describe('buildPagePptx text-box height fix', () => {
  ...
  it('pushes a lower text box down when an undersized title grows over it', async () => {
    const spec: PageSpec = {
      elements: [
        bigTitle({ y: 100, h: 30 }),
        {
          ...
          w: 600,
          h: 40,
          paragraphs: [{ runs: [{ text: 'subtitle', sizePt: 14 }] }],
        },
      ],
    }
    const { bytes } = await buildPagePptx(spec, deps)
    const opened = await openPptx(bytes)
    const els = opened.deck.slides[0]!.elements.filter((e): e is TextElement => e.type === 'text')
    expect(els).toHaveLength(2)
    const [title, sub] = [...els].sort((a, b) => a.transform.offset.y - b.transform.offset.y)
    expect(px(title!.transform.offset.cy)).toBeCloseTo(oneLinePx, 0)
    expect(px(sub!.transform.offset.y)).toBeGreaterThanOrEqual(
      px(title!.transform.offset.y + title!.transform.offset.cy) + 7,
    )
  })
