...
describe('anti hand-building from scratch', () => {
  ...
  it('title-only slide (1 text element) + add_text_box → allowed (rewriting a cover is not from-scratch)', async () => {
    const titleOnly = {
      widthPx: 1280,
      heightPx: 720,
      nodes: [textNode('e_2', '大家好！！！')],
    } as unknown as RenderSlide
    const r = await createSlidesSkill(mkAccess([titleOnly])).executeTool!(call('add_text_box'))
    expect(r.isError).toBeUndefined()
    expect((window as any).slidesApi.addElement).toHaveBeenCalledOnce()
  })
  it('layout-chrome text only + add_text_box → allowed (overlay page-local text)', async () => {
    const chromeOnly = {
      widthPx: 1280,
      heightPx: 720,
      nodes: [{ ...textNode('e_2', '大家好！！！'), decoration: true }],
    } as unknown as RenderSlide
    const r = await createSlidesSkill(mkAccess([chromeOnly])).executeTool!(call('add_text_box'))
    expect(r.isError).toBeUndefined()
    expect((window as any).slidesApi.addElement).toHaveBeenCalledOnce()
  })
  it('layout-chrome text only + add_shape → still refused (do not hand-build the page)', async () => {
    const chromeOnly = {
      widthPx: 1280,
      heightPx: 720,
      nodes: [{ ...textNode('e_2', '大家好！！！'), decoration: true }],
    } as unknown as RenderSlide
    const r = await createSlidesSkill(mkAccess([chromeOnly])).executeTool!(call('add_shape'))
    expect(r.isError).toBe(true)
    expect((window as any).slidesApi.addElement).not.toHaveBeenCalled()
  })
