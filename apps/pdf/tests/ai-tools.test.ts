...
function makeDeps(over: Partial<PdfAiDeps> = {}): PdfAiDeps {
  const overlays: Array<{ id: string; input: import('../src/shared/ipc').TextInsertInput }> = []
  return {
    ...
    insertText: vi.fn((input) => {
      overlays.push({ id: `ov${overlays.length + 1}`, input })
    }),
    listTextInserts: () => overlays.map((item) => ({ ...item })),
    updateTextInsert: vi.fn((id, input) => {
      const i = overlays.findIndex((item) => item.id === id)
      if (i < 0) return false
      overlays[i] = { id, input }
      return true
    }),
    deleteTextInsert: vi.fn((id) => {
      const i = overlays.findIndex((item) => item.id === id)
      if (i < 0) return false
      overlays.splice(i, 1)
      return true
    ...
    insertBlankPage: vi.fn(async (afterOrigIdx: number) => ({
      ok: true,
      newPage: afterOrigIdx + 2,
    })),
    ...over,
  }
}
...
describe('read_pages', () => {
  ...
  it('includes overlay inserts when the PDF content stream is empty', async () => {
    const result = await executePdfTool(
      makeDeps({
        doc: () => fakeDoc(['', '']),
        insertedText: (origIdx) => (origIdx === 0 ? 'vivo X500 系列新品发布会' : ''),
      }),
      call('read_pages', { start: 1 }),
    )
    expect(result.isError).toBeUndefined()
    expect(result.output).toContain('[Page 1]')
    expect(result.output).toContain('vivo X500 系列新品发布会')
    expect(result.output).not.toContain('No extractable text')
  })

  it('does not replace overlay text with empty OCR on a blank content stream', async () => {
    const result = await executePdfTool(
      makeDeps({
        doc: () => fakeDoc(['']),
        pageCount: () => 1,
        ocrText: () => 'OCR noise',
        insertedText: () => 'overlay body',
      }),
      call('read_pages', { start: 1 }),
    )
    expect(result.output).toContain('overlay body')
    expect(result.output).not.toContain('OCR noise')
  })
})
...
describe('search_text', () => {
  ...
  it('finds overlay insert text that is not in the pdf.js index', async () => {
    const result = await executePdfTool(
      makeDeps({
        insertedText: (origIdx) => (origIdx === 0 ? '影像新纪元 · 超越所见' : ''),
      }),
      call('search_text', { query: '影像新纪元' }),
    )
    expect(result.isError).toBeUndefined()
    expect(result.output).toContain('Page 1')
    expect(result.output).toContain('影像新纪元')
  })
})
...
describe('insert_text', () => {
  ...
  it('collapses stacked backslash-n escapes and <br> into line breaks', async () => {
    const deps = makeDeps()
    await executePdfTool(
      deps,
      call('insert_text', { page: 1, text: 'Hello\\\\nWorld<br>Next', x: 100, y: 72 }),
    )
    expect(deps.insertText).toHaveBeenCalledWith(
      expect.objectContaining({ text: 'Hello\nWorld\nNext' }),
    )
  ...
  it('wraps to the remaining page width when max_width is omitted', async () => {
    const deps = makeDeps()
    await executePdfTool(
      deps,
      call('insert_text', { page: 1, text: 'a'.repeat(200), x: 0, y: 72, font_size: 14 }),
    )
    const input = vi.mocked(deps.insertText).mock.calls[0]![0]
    const lines = input.text.split('\n')
    expect(lines.length).toBeGreaterThan(1)
    // fallback measure is 0.5×fontSize per ASCII char → 7pt; remaining width ~564pt
...
describe('create_document', () => {
  ...
  it('treats a missing bridge result as failure instead of throwing', async () => {
    const deps = makeDeps({ createDocument: vi.fn(async () => null as never) })
    const result = await executePdfTool(
      deps,
      call('create_document', { title: 'T', content: '<p>x</p>' }),
    )
    expect(result.isError).toBe(true)
    expect(result.output).toContain('creating the document failed')
  })
...
describe('insert_blank_page', () => {
  it('appends after the last page by default', async () => {
    const deps = makeDeps()
    const result = await executePdfTool(deps, call('insert_blank_page', {}))
    expect(result.isError).toBeUndefined()
    expect(deps.insertBlankPage).toHaveBeenCalledWith(1)
    expect(result.output).toContain('new page is 3')
  })

  it('inserts at the front when after_page is 0', async () => {
    const deps = makeDeps()
    const result = await executePdfTool(deps, call('insert_blank_page', { after_page: 0 }))
    expect(result.isError).toBeUndefined()
    expect(deps.insertBlankPage).toHaveBeenCalledWith(-1)
    expect(result.output).toContain('new page is 1')
  })

  it('rejects an out-of-range after_page', async () => {
    const deps = makeDeps()
    const result = await executePdfTool(deps, call('insert_blank_page', { after_page: 9 }))
    expect(result.isError).toBe(true)
    expect(deps.insertBlankPage).not.toHaveBeenCalled()
  })
})

describe('overlay text inserts', () => {
  it('lists, updates, and deletes overlay blocks on the current PDF', async () => {
    const deps = makeDeps()
    const inserted = await executePdfTool(
      deps,
      call('insert_text', { page: 1, text: '第一章 活动概述', x: 36, y: 72 }),
    )
    expect(inserted.isError).toBeUndefined()
    const listed = await executePdfTool(deps, call('list_text_inserts', {}))
    expect(listed.output).toContain('#1')
    expect(listed.output).toContain('第一章 活动概述')

    const updated = await executePdfTool(
      deps,
      call('update_text_insert', { insert_id: 1, text: '第一至四章', page: 2, x: 36, y: 72 }),
    )
    expect(updated.isError).toBeUndefined()
    expect(updated.output).toContain('page 2')
    expect(deps.updateTextInsert).toHaveBeenCalled()
    expect(deps.listTextInserts()[0]?.input.pageIndex).toBe(1)

    const deleted = await executePdfTool(deps, call('delete_text_insert', { insert_id: 1 }))
    expect(deleted.isError).toBeUndefined()
    expect(deps.listTextInserts()).toHaveLength(0)
  })

  it('rejects an unknown insert_id', async () => {
    const result = await executePdfTool(makeDeps(), call('update_text_insert', { insert_id: 1, text: 'x' }))
    expect(result.isError).toBe(true)
  })
})
