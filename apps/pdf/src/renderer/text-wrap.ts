...
/** Stay inside the printable page: 0.5 in margin when wrapping to remaining width. */
export const INSERT_PAGE_MARGIN_PT = 36
export const MIN_INSERT_FONT_PT = 8

export interface FitInsertLayoutOpts {
  text: string
  fontSize: number
  cssFamily: string
  cssStyle?: string
  pageW: number
  pageH: number
  /** Tentative left edge; omit to wrap to the full content width then center. */
  x?: number
  /** Tentative top edge; omit for the default 72 pt from the top. */
  y?: number
  maxWidth?: number
  margin?: number
  minFont?: number
...
export interface FitInsertLayout {
  lines: string[]
  fontSize: number
  lineLeading: number
  lineWidths: number[]
...
/** Wrap inserted text to the remaining page width and shrink the font if the
 *  block would run off the bottom. Always keeps the block inside the page box. */
export function fitInsertLayout(opts: FitInsertLayoutOpts): FitInsertLayout {
  const margin = opts.margin ?? INSERT_PAGE_MARGIN_PT
  const minFont = opts.minFont ?? MIN_INSERT_FONT_PT
  const cssStyle = opts.cssStyle ?? ''
  const centered = opts.x === undefined
  const ty0 = opts.y === undefined ? 72 : opts.y
  const maxLeft = Math.max(0, opts.pageW - margin - 24)
  const tx0 = centered ? margin : Math.min(Math.max(opts.x!, 0), maxLeft)
  const availW = Math.max(24, opts.pageW - tx0 - margin)
  const wrapW =
    opts.maxWidth !== undefined && Number.isFinite(opts.maxWidth) && opts.maxWidth > 0
      ? Math.min(opts.maxWidth, availW)
      : availW

  const layoutAt = (fs: number) => {
    const leading = fs * 1.2
    const lines = opts.text.split('\n').flatMap((p) =>
      p.trim() ? wrapText(p, wrapW, fs, opts.cssFamily, cssStyle) : [''],
    )
    const lineWidths = lines.map((l) => measurePt(l, fs, opts.cssFamily, cssStyle))
    const contentW = Math.max(...lineWidths, 1)
    const blockW = Math.min(wrapW, contentW)
    const blockH = leading * Math.max(0, lines.length - 1) + fs * 1.2
    return { lines, leading, lineWidths, blockW, blockH }
  }

  let fontSize = Math.max(minFont, opts.fontSize)
  let laid = layoutAt(fontSize)
  const availH = Math.max(fontSize * 1.2, opts.pageH - Math.max(ty0, 0) - margin)
  while (laid.blockH > availH + 0.5 && fontSize > minFont) {
    fontSize -= 1
    laid = layoutAt(fontSize)
  }

  const tx = centered
    ? (opts.pageW - laid.blockW) / 2
    : Math.min(Math.max(tx0, 0), Math.max(opts.pageW - laid.blockW, 0))
  const ty = Math.min(Math.max(ty0, 0), Math.max(opts.pageH - laid.blockH, 0))
  return {
    lines: laid.lines,
    fontSize,
    lineLeading: laid.leading,
    lineWidths: laid.lineWidths,
    blockW: laid.blockW,
    blockH: laid.blockH,
    x: tx,
    y: ty,
