...
import { fitInsertLayout, joinBlockLines, measurePt, wrapText } from '../text-wrap'
...
export interface PdfAiDeps {
  ...
  /** Visible overlay inserts on a page (AI/user add-text). Empty when none.
      These are not in the PDF content stream, so getTextContent cannot see them. */
  insertedText(origIdx: number): string
  ...
  /** Overlay inserts currently on the document (AI/user add-text), including unsaved ones */
  listTextInserts(): Array<{ id: string; input: TextInsertInput }>
  /** Replace an overlay insert in place; returns false if id is unknown */
  updateTextInsert(id: string, input: TextInsertInput): boolean
  /** Remove an overlay insert; returns false if id is unknown */
  ...
  /**
   * Insert a blank page after the given original page index (-1 = before page 1).
   * Flushes pending overlays then writes the file (same as the ribbon action).
   */
  insertBlankPage(afterOrigIdx: number): Promise<{ ok: boolean; error?: string; newPage?: number }>
}
...
export const AGENT_TOOLS: AgentToolDef[] = [
  {
    name: 'read_pages',
    description:
      'Read the text content of a page range (with [Page N] markers). Includes native PDF text and visible overlay inserts (AI-generated / add-text). Read the relevant pages before answering questions; at most 10 pages per call, over-long output is truncated.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'search_text',
    description:
      'Search the full text for a string (native PDF text and visible overlay inserts); returns the page number and a context excerpt for each hit. Prefer this when locating which page something is on.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'edit_text',
    description:
      'Replace a short text run on a page (rewrites native PDF content-stream text; takes effect on save). Does NOT edit overlay inserts from insert_text — use list_text_inserts and update_text_insert for those. old_text must be a verbatim fragment that actually exists on that page (confirm with read_pages or search_text first); only the first occurrence on the page is edited unless occurrence is given. The replacement is drawn from the original position without reflowing the page, so keep it close to the original length; text cannot be deleted (new_text must not be empty).',
    inputSchema: {
      type: 'object',
      properties: {
        page: { type: 'integer', description: 'Page number (1-based)' },
  ...
  },
  {
    name: 'insert_text',
    description:
      'Add NEW overlay text to a page (takes effect on save). Use for blank pages, empty areas, and filling non-interactive form blanks. To change overlay text already on the page, use list_text_inserts then update_text_insert / delete_text_insert — not edit_text (that only edits native PDF content-stream text). Position with anchor_text plus placement, or x/y as the TOP-LEFT of the block from the page top-left; with neither, the block is centered near the top. Real newlines or "\\n" start a new line. Long lines wrap to the remaining page width; pass max_width for a column/card/cell. One insert_text is one visual block.',
    inputSchema: {
      type: 'object',
      properties: {
        page: { type: 'integer', description: 'Page number (1-based)' },
        text: {
          type: 'string',
          description:
            'Text to insert. Use real newlines (or the two-character sequence \\n) between lines; do not put a visible backslash-n in the output.',
        },
        anchor_text: {
          type: 'string',
          description:
        ...
        max_width: {
          type: 'number',
          description:
            'Wrap width in points (column/card/cell). Omit to wrap to the remaining page width so text cannot run off the right edge.',
        },
        align: {
          type: 'string',
  ...
  {
    name: 'list_text_inserts',
    description:
      'List overlay text blocks on this PDF (insert_text / 插入文字), with 1-based insert_id, page, display x/y, font size, and a text preview. Call this before update_text_insert or delete_text_insert. These blocks are the visible body when the native page is empty or the user drafted with AI.',
    inputSchema: {
      type: 'object',
      properties: {
        page: {
          type: 'integer',
          description: 'Page number (1-based); omit to list overlays on every page',
        },
      },
    },
  },
  {
    name: 'update_text_insert',
    description:
      'Rewrite, move, or restyle an existing overlay text block in THIS PDF (the original document). insert_id comes from list_text_inserts. Omitted fields keep their current value. Set page (and optionally x/y) to move the block onto another page — use insert_blank_page first if that page does not exist yet. To split overflowing text: shorten this block with text=, then insert_text on the new page for the remainder.',
    inputSchema: {
      type: 'object',
      properties: {
        insert_id: {
          type: 'integer',
          description: 'Overlay number from list_text_inserts (1-based)',
        },
        text: { type: 'string', description: 'Replacement text; omit to keep the current text' },
        page: { type: 'integer', description: 'Move to this 1-based page; omit to stay' },
        ...
        max_width: { type: 'number', description: 'Wrap width in points' },
  ...
  {
    name: 'delete_text_insert',
    description:
      'Remove an overlay text block from THIS PDF. insert_id comes from list_text_inserts. Does not affect native PDF content-stream text.',
    inputSchema: {
      type: 'object',
      properties: {
        insert_id: {
  ...
  },
  {
    ...
    description:
      'Insert a blank page into THIS PDF (same size as the neighbor). Use when content does not fit the current page — PDF pages do not grow or reflow. after_page is 1-based (0 = insert at the front; omit to append after the last page). Then write with insert_text on the new page; never stack new text over existing content.',
    inputSchema: {
      type: 'object',
      properties: {
        after_page: {
          type: 'integer',
          description: 'Insert after this 1-based page (0 = before page 1; default = last page)',
        },
      },
    },
  },
...
function overlaySnippet(text: string, query: string): string {
  const pos = text.toLowerCase().indexOf(query.toLowerCase())
  if (pos < 0) return text.slice(0, 80).replace(/\s+/g, ' ')
  const from = Math.max(0, pos - 40)
  return text.slice(from, pos + query.length + 40).replace(/\s+/g, ' ')
}
...
async function readPages(deps: PdfAiDeps, input: Record<string, unknown>): Promise<ToolExecution> {
  ...
  for (let n = start; n <= Math.min(end, doc.numPages); n++) {
    ...
    page.cleanup()
    const overlay = deps.insertedText(n - 1).trim()
    if (isScannedText(text) && !overlay) {
      const ocr = deps.ocrText(n - 1)
      if (ocr && ocr.trim()) {
        text = `(recovered by OCR; may contain recognition errors)\n${ocr}`
      }
    }
    if (overlay) text = text.trim() ? `${text.trim()}\n\n${overlay}` : overlay
    out += `[Page ${n}]\n${text.trim()}\n\n`
...
async function searchText(deps: PdfAiDeps, input: Record<string, unknown>): Promise<ToolExecution> {
  ...
  for (const m of matches.slice(0, 40)) {
    ...
    hitPages.add(m.pageIndex)
  }
  let overlayHits = 0
  for (let i = 0; i < deps.pageCount(); i++) {
    if (deps.isDeleted(i) || hitPages.has(i)) continue
    const overlay = deps.insertedText(i)
    if (!overlay.toLowerCase().includes(query.toLowerCase())) continue
    overlayHits++
    if (lines.length < 40) {
      lines.push(`Page ${i + 1}: …${overlaySnippet(overlay, query)}…`)
    }
  }
  const count = matches.length + overlayHits
  if (count > 40) lines.push(`(${count} matches total; only the first 40 listed)`)
  return {
    output: lines.join('\n') || 'No matches found',
    summary: t('aiToolSearch', { query, count }),
  }
}
...
function rgb255Hex(c: readonly [number, number, number]): string {
  return `#${c.map((v) => Math.max(0, Math.min(255, Math.round(v))).toString(16).padStart(2, '0')).join('')}`
}

function insertTopLeft(geom: PageGeom, input: TextInsertInput): [number, number] {
  const [vx, vy] = pdfToView(geom, input.origin[0], input.origin[1])
  return [vx, vy - input.fontSize]
}

export function sortedOverlayInserts(deps: PdfAiDeps): Array<{ id: string; input: TextInsertInput }> {
  return [...deps.listTextInserts()].sort((a, b) => {
    if (a.input.pageIndex !== b.input.pageIndex) return a.input.pageIndex - b.input.pageIndex
    const ga = deps.pageGeom(a.input.pageIndex)
    const gb = deps.pageGeom(b.input.pageIndex)
    if (!ga || !gb) return 0
    const [ax, ay] = insertTopLeft(ga, a.input)
    const [bx, by] = insertTopLeft(gb, b.input)
    return ay - by || ax - bx
  })
}
...
function resolveOverlayInsert(
  deps: PdfAiDeps,
  raw: unknown,
): { item: { id: string; input: TextInsertInput }; n: number } | { bad: string } {
  const list = sortedOverlayInserts(deps)
  const n = Number(raw)
  if (!Number.isInteger(n) || n < 1 || n > list.length) {
    return {
      bad: `insert_id ${String(raw)} is out of range (document has ${list.length} overlay insert(s); call list_text_inserts)`,
    }
  }
  return { item: list[n - 1]!, n }
}

async function layoutOverlayInsert(
  deps: PdfAiDeps,
  origIdx: number,
  input: Record<string, unknown>,
  defaults: {
...
> {
  const geom = deps.pageGeom(origIdx)
  if (!geom) return { bad: 'Document not ready' }
  const text = decodeToolNewlines(String(input.text ?? defaults.text)).trim()
  if (!text) return { bad: 'text must not be empty' }

  const requestedSize = input.font_size === undefined ? defaults.fontSize : Number(input.font_size)
  if (!(requestedSize > 0)) return { bad: 'font_size must be a positive number' }
  let color = defaults.color
  if (input.color !== undefined) {
    const hex = HEX_COLOR.exec(String(input.color))
    if (!hex) return { bad: `Invalid color "${String(input.color)}"; use #RRGGBB` }
    const v = parseInt(hex[1]!, 16)
    color = [(v >> 16) & 255, (v >> 8) & 255, v & 255]
  }
  const font = input.font === undefined ? defaults.font : String(input.font)
  ...
  const alignRaw = input.align === undefined ? defaults.align : String(input.align)
  if (alignRaw !== 'left' && alignRaw !== 'center' && alignRaw !== 'right')
    return { bad: "align must be 'left', 'center' or 'right'" }
  const align = alignRaw
  const maxWidth = input.max_width === undefined ? undefined : Number(input.max_width)
  if (maxWidth !== undefined && !(maxWidth > 0)) return { bad: 'max_width must be a positive number' }

  const bold = input.bold === undefined ? defaults.bold : input.bold === true ? true : undefined
  const italic = input.italic === undefined ? defaults.italic : input.italic === true ? true : undefined
  const cssFamily =
  ...
  let tx: number
  let ty: number
  let fitted: ReturnType<typeof fitInsertLayout>
  const anchor = String(input.anchor_text ?? '').trim()
  if (anchor) {
    const indexPromise = deps.searchIndex()
    ...
    if (!located) {
      return {
        bad: `"${anchor}" not found on page ${origIdx + 1}; use read_pages to verify the exact text`,
      }
    }
    const a = dispBox(geom, located.rect)
    const placement = String(input.placement ?? 'right')
    const ax = placement === 'right' ? a.left + a.width + TEXT_GAP_PT : a.left
    fitted = fitInsertLayout({ ...fitBase, x: ax, y: a.top })
    tx = fitted.x
    if (placement === 'above') ty = a.top - TEXT_GAP_PT - fitted.blockH
    else if (placement === 'below') ty = a.top + a.height + TEXT_GAP_PT
    else ty = a.top + a.height - fitted.fontSize * 1.1
    ty = Math.min(Math.max(ty, 0), Math.max(size.height - fitted.blockH, 0))
  } else {
    if (input.x !== undefined && !Number.isFinite(Number(input.x)))
      return { bad: 'x and y must be numbers (points from the page top-left as displayed)' }
    if (input.y !== undefined && !Number.isFinite(Number(input.y)))
      return { bad: 'x and y must be numbers (points from the page top-left as displayed)' }
    fitted = fitInsertLayout({
      ...fitBase,
      x: input.x === undefined ? defaults.x : Number(input.x),
      y: input.y === undefined ? defaults.y : Number(input.y),
    })
    tx = fitted.x
    ty = fitted.y
  }

  const { lines, lineWidths, blockW, blockH, lineLeading, fontSize } = fitted
  return {
    payload: {
      pageIndex: origIdx,
...
async function insertTextTool(
  ...
  const r = resolvePage(deps, input.page)
  if ('bad' in r) return err(r.bad, summary)
  const laid = await layoutOverlayInsert(
    deps,
    r.origIdx,
    input,
    { text: '', fontSize: 14, color: [0, 0, 0], align: 'left' },
    summary,
  )
  if ('bad' in laid) return err(laid.bad, summary)
  if (signal?.aborted) return err('stopped by the user; nothing was changed', summary)
  deps.insertText(laid.payload)
  deps.gotoPage(r.origIdx + 1)
  const shrunk =
    laid.fontSize + 0.5 < laid.requestedSize
      ? `, font reduced to ${fmt(laid.fontSize)} pt to fit the page`
      : ''
  const preview = laid.text.length > 400 ? `${laid.text.slice(0, 400)}…` : laid.text
  return {
    output: `Inserted ${laid.lines.length} line(s) of text on page ${r.origIdx + 1} at x=${fmt(laid.tx)}, y=${fmt(laid.ty)}, ${fmt(laid.fontSize)} pt, occupying ${fmt(laid.blockW)}×${fmt(laid.blockH)} pt on a ${fmt(laid.size.width)}×${fmt(laid.size.height)} pt page${shrunk}. Text:\n${preview}\nPlace further inserts outside this box (unsaved; the user can drag/edit it, undo with ⌘Z, save with ⌘S).`,
    mutated: true,
    summary,
  }
}

function listTextInsertsTool(deps: PdfAiDeps, input: Record<string, unknown>): ToolExecution {
  const summary = t('insertText')
  let list = sortedOverlayInserts(deps)
  if (input.page !== undefined) {
    const r = resolvePage(deps, input.page)
    if ('bad' in r) return err(r.bad, summary)
    list = list.filter((item) => item.input.pageIndex === r.origIdx)
  }
  if (list.length === 0) {
    return {
      output: 'No overlay text inserts. Use insert_text to add new blocks to this PDF.',
      summary,
    }
  }
  const lines = list.map((item, i) => {
    const geom = deps.pageGeom(item.input.pageIndex)
    const [x, y] = geom ? insertTopLeft(geom, item.input) : [0, 0]
    const preview =
      item.input.text.length > 180 ? `${item.input.text.slice(0, 180)}…` : item.input.text
    return `#${i + 1} page ${item.input.pageIndex + 1} x=${fmt(x)} y=${fmt(y)} ${fmt(item.input.fontSize)}pt ${rgb255Hex(item.input.color)}\n${preview}`
  })
  return {
    output: `${list.length} overlay insert(s). Use update_text_insert / delete_text_insert with insert_id.\n\n${lines.join('\n\n')}`,
    summary,
  }
}

async function updateTextInsertTool(
  deps: PdfAiDeps,
  input: Record<string, unknown>,
  signal?: AbortSignal,
): Promise<ToolExecution> {
  const summary = t('editInsertedText')
  if (deps.readOnly()) return err(READONLY_OUTPUT, summary)
  const found = resolveOverlayInsert(deps, input.insert_id)
  if ('bad' in found) return err(found.bad, summary)
  const cur = found.item.input
  let origIdx = cur.pageIndex
  if (input.page !== undefined) {
    const r = resolvePage(deps, input.page)
    if ('bad' in r) return err(r.bad, summary)
    origIdx = r.origIdx
  }
  const geom = deps.pageGeom(cur.pageIndex)
  const [cx, cy] = geom ? insertTopLeft(geom, cur) : [36, 36]
  const laid = await layoutOverlayInsert(
    deps,
    origIdx,
    input,
  ...
  if ('bad' in laid) return err(laid.bad, summary)
  if (signal?.aborted) return err('stopped by the user; nothing was changed', summary)
  if (!deps.updateTextInsert(found.item.id, laid.payload)) {
    return err('overlay insert could not be updated (it may have been deleted)', summary)
  }
  deps.gotoPage(origIdx + 1)
  const preview = laid.text.length > 400 ? `${laid.text.slice(0, 400)}…` : laid.text
  return {
    output: `Updated overlay #${found.n} on page ${origIdx + 1} at x=${fmt(laid.tx)}, y=${fmt(laid.ty)}, ${fmt(laid.fontSize)} pt (${laid.lines.length} line(s)). Text:\n${preview}\nUnsaved; the user can undo with ⌘Z and save with ⌘S.`,
    mutated: true,
    summary,
  }
}

function deleteTextInsertTool(deps: PdfAiDeps, input: Record<string, unknown>): ToolExecution {
  const summary = t('deleteInsertedText')
  if (deps.readOnly()) return err(READONLY_OUTPUT, summary)
  const found = resolveOverlayInsert(deps, input.insert_id)
  if ('bad' in found) return err(found.bad, summary)
  if (!deps.deleteTextInsert(found.item.id)) {
    return err('overlay insert could not be deleted', summary)
  }
  const preview =
    found.item.input.text.length > 120
      ? `${found.item.input.text.slice(0, 120)}…`
      : found.item.input.text
  return {
    output: `Deleted overlay #${found.n} from page ${found.item.input.pageIndex + 1}: "${preview}"`,
    mutated: true,
    summary,
  }
}
...
export async function executePdfTool(
  ...
  switch (call.name) {
    ...
    case 'list_text_inserts':
      return listTextInsertsTool(deps, input)
    case 'update_text_insert':
      return updateTextInsertTool(deps, input, signal)
    case 'delete_text_insert':
      return deleteTextInsertTool(deps, input)
    ...
    case 'insert_blank_page': {
      const summary = t('insertBlankPage')
      if (deps.readOnly()) return err(READONLY_OUTPUT, summary)
      const last = deps.pageCount()
      const afterRaw = input.after_page === undefined ? last : Number(input.after_page)
      if (!Number.isInteger(afterRaw) || afterRaw < 0 || afterRaw > last) {
        return err(`after_page must be 0..${last} (0 = front)`, summary)
      }
      if (afterRaw >= 1) {
        const r = resolvePage(deps, afterRaw)
        if ('bad' in r) return err(r.bad, summary)
      }
      const inserted = await deps.insertBlankPage(afterRaw - 1)
      if (!inserted.ok) return err(inserted.error ?? 'inserting a blank page failed', summary)
      const newPage = inserted.newPage ?? afterRaw + 1
      return {
        output: `Inserted a blank page after page ${afterRaw || 0}; new page is ${newPage}. Use insert_text on page ${newPage}. Document now has ${deps.pageCount()} pages.`,
        mutated: true,
        summary,
      }
    }
    ...
    case 'create_document': {
      ...
      if (!r || !r.ok) return err(r?.error ?? 'creating the document failed', summary)
