...
describe('updateTextStyle', () => {
  ...
  it('CJK font pick writes both slots so mixed ASCII follows; Latin pick keeps the CJK font', () => {
    const mixed = () =>
      createEditor([
        para([
          text('合同 Contract', [
    ...
    expect(textStyleOf(cjk, 0)).toMatchObject({ font: 'KaiTi', fontAscii: 'KaiTi' })
...
describe('setHeadingLevel', () => {
  it('demotes a matched heading level and restamps the heading pStyle', () => {
    const editor = createEditor(fixtureDoc())
    const outcome = executeCommands(editor, {
      commands: [
        {
    ...
    expect(node.attrs.styleId).toBe('Heading3')
  ...
  it('promotes a paragraph to a heading', () => {
    ...
    expect(node.attrs.styleId).toBe('Heading2')
  })
})
