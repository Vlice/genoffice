...
describe('insert table on a blank document', () => {
  it('ensures leading and trailing paragraphs so the caret can leave the table', async () => {
    const { editor } = await openBlank()
    insertTableAt(editor, 1, 2)
    const top = editor.getJSON().content ?? []
    const types = top.map((n: { type?: string }) => n.type)
    expect(types[0]).toBe('docParagraph')
    expect(types).toContain('docTable')
    expect(types[types.length - 1]).toBe('docParagraph')
    // at least one textblock sibling on each side of the table
    const tableIdx = types.indexOf('docTable')
    expect(tableIdx).toBeGreaterThan(0)
    expect(tableIdx).toBeLessThan(types.length - 1)
    editor.destroy()
  })
})
