...
function tableCellSpec(tag: 'td' | 'th', node: PmNode): DOMOutputSpec {
  ...
  // Auto-height rows: CSS vertical-align alone is a no-op when the cell hugs
  // content. Wrap center/bottom cells so align-content has a min box to work in.
  const vAlign = node.attrs.vAlign as string | null
  if (vAlign === 'center' || vAlign === 'bottom') {
    return [tag, attrs, ['div', { class: 'cell-valign', 'data-valign': vAlign }, 0]]
  }
  return [tag, attrs, 0]
}
...
/** Stamp the gallery fill and stroke onto each run so the glyph matches the preset. */
function applyWordArtEffect(
  doc: Record<string, unknown>,
  effect: string | undefined,
): Record<string, unknown> {
  if (!effect) return doc
  const paras = doc.content as Array<Record<string, unknown>> | undefined
  if (!paras) return doc
  for (const para of paras) {
    const inlines = para.content as Array<Record<string, unknown>> | undefined
    if (!inlines) continue
    for (const node of inlines) {
      ...
      const style = marks.find((mark) => mark.type === 'docTextStyle')
      if (style) style.attrs = { ...(style.attrs ?? {}), textEffect: effect }
      else marks.push({ type: 'docTextStyle', attrs: { textEffect: effect } })
      node.marks = marks
    }
  }
  return doc
...
function textboxDocJson(box: TextboxDisplay): Record<string, unknown> {
  ...
  const emptyPara = fresh
    ...
    : { type: 'docParagraph' }
  return applyWordArtEffect(
    { type: 'doc', content: paras.length > 0 ? paras : [emptyPara] },
    wordArtGlyphCss(box),
  )
}
...
function mountTextboxEditors(
  ...
  const painters: Array<(() => void) | undefined> = []
  ...
  boxEls.forEach((el, i) => {
    ...
    // TipTap 3's default `element: el` nests a child .ProseMirror inside the
    // host. WordArt paint (-webkit-text-fill / stroke) lives on .doc-textbox, so
    // nested glyphs lose the preset (plain black / empty / stroke ghost). Mount
    // the editor onto the box itself so styles and text share one node.
    // Declared before the editor so a synchronous init transaction does not
    // touch it while it is still uninitialized. Paint only after mount.
    let editorView: EditorView | undefined
    const paint = () => {
      const current = knownBoxes?.[i]
      if (!current || !editorView) return
      const observer = (
        editorView as EditorView & { domObserver?: { stop: () => void; start: () => void } }
      ).domObserver
      // Stamping span styles is not an edit; a live observer would read it back
      // into the document.
      observer?.stop()
      try {
        paintWordArtGlyphs(el, current)
      } finally {
        observer?.start()
      }
    }
    const sub: Editor = new Editor({
      element: { mount: el } as HTMLElement,
      ...
      onTransaction: ({ transaction }) => {
        ...
        // selection and typing both rebuild spans; restamp so the focused
        // glyph matches the idle one (run color alone is the solid fallback)
        paint()
      },
    })
    ...
    if (box.wordArtId) el.setAttribute('data-wordart', box.wordArtId)
    else if (box.textOutline) el.setAttribute('data-wordart', 'stroke')
    else el.removeAttribute('data-wordart')
    editorView = sub.view
    painters[i] = paint
    paint()
    editors[i] = sub
 ...
 * Inline images (imageWrap null / 嵌入型) may re-anchor to another paragraph
 * on drop, but never auto-convert to square wrap — wrap changes only via the
 * Wrap Text UI (ribbon / context menu).
 */
        ...
        mousedown: (view, event) => {
          ...
          const onUp = (e: MouseEvent) => {
            ...
            if (isTextbox) {
              ...
              // 嵌入型: keep wp:inline. Drag may re-home the atom to another
              // paragraph; inventing square wrap / posOffset here is what made
              // layout jump to 四周型环绕 after a casual move.
              if (prev == null) {
                if (
                  paragraphAnchor &&
              ...
              // Floating pictures keep their wrap kind; square/tight/through
              // re-pick left/right from the drop X (Word wraps on the open side).
              const kindMatch = /^(square|tight|through)-(?:left|right)$/.exec(prev)
              let nextWrap = prev
              if (kindMatch) {
