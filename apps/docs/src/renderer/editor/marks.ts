...
import { toggleStyleAwareMark } from './text-style-resolve'
...
export const BoldMark = Mark.create({
  ...
  addKeyboardShortcuts() {
    return { 'Mod-b': () => toggleStyleAwareMark(this.editor, 'bold') }
  },
})
...
export const ItalicMark = Mark.create({
  ...
  addKeyboardShortcuts() {
    return { 'Mod-i': () => toggleStyleAwareMark(this.editor, 'italic') }
  },
})
...
export const TextStyleMark = Mark.create({
  ...
  addAttributes() {
    return {
      ...
      // WordArt fill + stroke painted on the glyph. Display-only: the saved
      // run color stays the solid fallback (see wordArtSolidColor).
      textEffect: { default: null as string | null, rendered: false },
    }
  },
  parseHTML() {
    return [
  ...
  renderHTML({ mark }) {
    ...
    // After the solid run color, so the preset fill and stroke win on the glyph.
    if (mark.attrs.textEffect) styles.push(String(mark.attrs.textEffect))
