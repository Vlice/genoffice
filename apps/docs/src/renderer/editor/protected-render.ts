...
/**
 * Outline as a text-shadow ring. Chrome drops `-webkit-text-stroke` while the
 * glyphs are in a selection (clicking the box selects them), so a white fill
 * vanishes on the page until the selection clears. The shadow ring stays.
 */
export function wordArtOutlineShadow(stroke: string | undefined, extra?: string): string {
  const parts: string[] = []
  const parsed = stroke ? /^([\d.]+)px\s+(.+)$/.exec(stroke.trim()) : null
  if (parsed) {
    const width = Number(parsed[1])
    const color = parsed[2]
    if (width > 0) {
      ...
      for (let i = 1; i <= rings; i++) {
        const radius = (width * i) / rings
        for (const [x, y] of dirs) {
          const px = (n: number) => `${Math.round(n * 100) / 100}px`
          parts.push(`${px(x * radius)} ${px(y * radius)} 0 ${color}`)
        }
      }
    }
  }
  if (extra && extra !== 'none') parts.push(extra)
  return parts.length > 0 ? parts.join(',') : 'none'
}

/**
 * Inline paint for one WordArt glyph host. The saved run keeps a readable
 * solid color (light fills become the outline color), so without this the
 * page shows that solid — white-on-red collapses into the solid red preset.
 * `!important` sits on the glyph itself: a stylesheet variable on the box
 * does not win against the run's inline color once ProseMirror owns the span.
 */
export function wordArtGlyphCss(box: TextboxDisplay): string | undefined {
  const wa = box.wordArtId ? WORDART_CSS[box.wordArtId] : undefined
  const fill = wa?.color
  const stroke =
    wa?.stroke ??
    (box.textOutline ? `${box.textOutline.widthPx}px #${box.textOutline.colorHex}` : undefined)
  if (!fill && !stroke && !wa?.textShadow) return undefined
  const fillValue = fill ?? 'currentColor'
  return [
    `color:${fillValue} !important`,
    `-webkit-text-fill-color:${fillValue} !important`,
    // Stroke is not painted for selected glyphs; the shadow ring is the outline.
    `-webkit-text-stroke:0px transparent !important`,
    `text-shadow:${wordArtOutlineShadow(stroke, wa?.textShadow)} !important`,
  ].join(';')
}

/** Re-stamp WordArt paint onto the live editor. ProseMirror rebuilds spans on each edit. */
export function paintWordArtGlyphs(root: HTMLElement, box: TextboxDisplay): void {
  const css = wordArtGlyphCss(box)
  if (!css) return
  const decls = css.split(';').map((decl) => {
    const cut = decl.indexOf(':')
    return {
      prop: decl.slice(0, cut).trim(),
      value: decl.slice(cut + 1).replace(/\s*!important\s*$/, '').trim(),
    }
  })
  ...
  apply(root)
  root.querySelectorAll('div, p, span, a').forEach((node) => apply(node as HTMLElement))
}

let wordArtPresetCssReady = false

/** Literal preset colors, so a missing custom property cannot fall through to the run's solid red. */
function ensureWordArtPresetCss(): void {
  if (wordArtPresetCssReady || typeof document === 'undefined') return
  wordArtPresetCssReady = true
  const rules: string[] = []
  for (const [id, wa] of Object.entries(WORDART_CSS)) {
    const sel = `.doc-textbox[data-wordart="${id}"]`
    const hosts = [
      sel,
      `${sel} .doc-textbox-editor`,
      `${sel} .doc-textbox-para`,
      `${sel} div`,
      `${sel} p`,
      `${sel} span`,
      `${sel} a`,
    ...
    rules.push(
      `${hosts.join(',')}{color:${wa.color} !important;-webkit-text-fill-color:${wa.color} !important;` +
        `-webkit-text-stroke:0px transparent !important;` +
        `text-shadow:${wordArtOutlineShadow(wa.stroke, wa.textShadow)} !important}`,
    )
  }
  const style = document.createElement('style')
  style.id = 'wordart-preset-paint'
  style.textContent = rules.join('\n')
  document.head.appendChild(style)
}

export function textboxBoxStyle(box: TextboxDisplay): string {
  if (box.wordArtId || box.textOutline) ensureWordArtPresetCss()
  ...
  const fillCss = waStyle?.color
  const strokeCss =
    waStyle?.stroke ??
    (box.textOutline ? `${box.textOutline.widthPx}px #${box.textOutline.colorHex}` : undefined)
  const outlineShadow =
    strokeCss || waStyle?.textShadow
      ? wordArtOutlineShadow(strokeCss, waStyle?.textShadow)
      : undefined
  // Same text-align on the box and the nested editor (TipTap mounts onto the
  // box itself). Otherwise selected WordArt looks left while the static spec
  // and the saved w:jc look centered.
  const paraAlign = box.paras[0]?.align
  const sharedAlign =
    paraAlign && box.paras.every((p) => (p.align ?? paraAlign) === paraAlign)
      ? paraAlign
      : undefined
  ...
  return [
    ...
    transforms.filter(Boolean).length > 0
      ...
      : '',
    fillCss ? `--wordart-fill:${fillCss};-webkit-text-fill-color:${fillCss}` : '',
    outlineShadow && outlineShadow !== 'none'
      ? `--wordart-shadow:${outlineShadow};text-shadow:${outlineShadow};-webkit-text-stroke:0px transparent`
      : '',
    ...
    sharedAlign ? `text-align:${sharedAlign}` : '',
...
export function runSpanSpecs(run: Run, autoSpace?: boolean, extraStyle?: string): DomSpec[] {
  const out: DomSpec[] = []
  ...
  if (run.text !== '' || !run.image) out.push(textSpanSpec(run, autoSpace, extraStyle))
  if (run.image) {
...
function textSpanSpec(run: Run, autoSpace?: boolean, extraStyle?: string): DomSpec {
...
function runSpansWithPads(runs: Run[], autoSpace?: boolean, extraStyle?: string): DomSpec[] {
  const out: DomSpec[] = []
  let prevText = ''
  for (const run of runs) {
    ...
    out.push(...runSpanSpecs(run, autoSpace, extraStyle))
  }
  return out
}
...
export function renderTextboxSpec(box: TextboxDisplay): DomSpec {
  ...
  if (box.wordArtId) boxAttrs['data-wordart'] = box.wordArtId
  else if (box.textOutline) boxAttrs['data-wordart'] = 'stroke'
  ...
  const glyphCss = wordArtGlyphCss(box)
  const paras: DomSpec[] = box.paras.map((para) => {
    const spans: DomSpec[] = runSpansWithPads(para.runs, para.autoSpace, glyphCss)
