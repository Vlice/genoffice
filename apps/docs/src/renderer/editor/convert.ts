      ...
      if (idx !== null && idx !== undefined && originalByIndex.has(idx) && !usedIndexes.has(idx)) {
        ...
        if ((imagePatch || imageReplace) && original.originalXml) {
          ...
          if (imagePatch?.wrap !== undefined) {
            // Rebuilding the wrap element also rebuilds positionH/V. Pass the
            // live posOffset (even when wrap-only changed) so a picture that
            // was dragged to the right does not snap to the wrap default (left).
            const posOffset = numericPosOffset(
              imagePatch.posOffsetX ??
                (node.attrs?.imageOffsetXEmu != null
                  ? Number(node.attrs.imageOffsetXEmu)
                  : original.imageOffsetXEmu),
              imagePatch.posOffsetY ??
                (node.attrs?.imageOffsetYEmu != null
                  ? Number(node.attrs.imageOffsetYEmu)
                  : original.imageOffsetYEmu),
            )
            const marginAlign =
            ...
            // posOffset changed without wrap change. patchImageParagraphXml rewrites
            // existing <wp:posOffset>, or converts <wp:align> → free offset (front/
            // behind wraps). If anything still looks align-based, rebuild via wrap.
            const stillAlign = /<wp:positionH[^>]*>[\s\S]*?<wp:align[\s/>]/.test(xml)
            const wrap =
              (node.attrs?.imageWrap as ImageWrap | null) ?? original.imageWrap ?? null
            if (stillAlign && wrap) {
              xml = applyImageWrap(
                xml,
                wrap,
        ...
        if (genWrap !== undefined && genWrap !== null) {
          xml = applyImageWrap(xml, genWrap, numericPosOffset(genOffsetX, genOffsetY))
        }
        pushBlock({ kind: 'xml', xml })
        ...
        applyLiveImagePosition(image, node.attrs)
        pushBlock({ kind: 'image', image })
...
/** Rebuild a NewImage from live editor attrs when no genImage / originalXml remains. */
function numericPosOffset(
  x: number | undefined,
  y: number | undefined,
...
/** Copy live editor position onto a NewImage so save matches the on-screen float. */
function applyLiveImagePosition(image: NewImage, attrs: Record<string, unknown> | undefined): void {
  const pos = numericPosOffset(
    attrs?.imageOffsetXEmu != null ? Number(attrs.imageOffsetXEmu) : undefined,
    attrs?.imageOffsetYEmu != null ? Number(attrs.imageOffsetYEmu) : undefined,
  )
  if (pos) image.posOffsetEmu = pos
}
  ...
  for (const node of content) {
    ...
    for (const mark of node.marks ?? []) {
      if (mark.type === 'bold') run.bold = true
        ...
        // Word w:b/w:i w:val="0": only write false when the bold/italic mark is absent
        if (mark.attrs?.boldOff && run.bold !== true) run.bold = false
        if (mark.attrs?.italicOff && run.italic !== true) run.italic = false
...
function runStyleKey(run: Run): string {
  return JSON.stringify([
    run.rawRPr ?? null,
    run.styleId ?? null,
    // tri-state: true / false (explicit w:val="0") / null (inherit) — !! collapses off+absent
    run.bold ?? null,
    run.italic ?? null,
    !!run.underline,
    !!run.strike,
...
function normalizedRuns(runs: Run[]): unknown[] {
  return mergeRuns(runs).map((r) => [
    ...
    r.rawRPr ?? null,
    r.styleId ?? null,
    // tri-state: true / false (explicit off) / null (inherit from style)
    r.bold ?? null,
    r.italic ?? null,
    !!r.underline,
    !!r.strike,
