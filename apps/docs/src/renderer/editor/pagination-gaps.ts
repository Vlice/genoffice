 ...
 */
/** Paper rects on the continuous canvas, skipping gray inter-page bands. */
export function pagePaperBounds(
  wrap: HTMLElement,
  zoomFactor: number,
  ...
  if (!pages.length) pages.push({ top: 0, height: wr.height / zoomFactor })
  return pages
}
...
export function syncPageBorders(
  ...
  const pages = pagePaperBounds(wrap, zoomFactor)
  pages.forEach((page, pageIdx) => {
    if (style.display === 'firstPage' && pageIdx > 0) return
    if (style.display === 'notFirstPage' && pageIdx === 0) return
    const el = document.createElement('div')
    el.className = 'page-border-overlay'
    el.style.top = `${page.top + inset.top}px`
    el.style.left = `${inset.left}px`
    el.style.right = `${inset.right}px`
    el.style.height = `${page.height - inset.top - inset.bottom}px`
...
/**
 * Text watermark as a repeating diagonal tile on each paper page (not one
 * ghost centered on the whole continuous canvas).
 */
export function syncPageWatermarks(
  wrap: HTMLElement,
  text: string | null,
  zoomFactor: number,
): void {
  let layer = wrap.querySelector(':scope > .page-watermark-overlays') as HTMLElement | null
  if (!text) {
    layer?.remove()
    return
  }
  if (!layer) {
    layer = document.createElement('div')
    layer.className = 'page-watermark-overlays'
    wrap.appendChild(layer)
  }
  const pages = pagePaperBounds(wrap, zoomFactor)
  const image = watermarkTileBackground(text)
  const size = `${WATERMARK_TILE_W}px ${WATERMARK_TILE_H}px`
  const kids = Array.from(layer.children) as HTMLElement[]
  while (kids.length > pages.length) kids.pop()?.remove()
  pages.forEach((page, i) => {
    let el = kids[i]
    if (!el) {
      el = document.createElement('div')
      el.className = 'page-watermark-page'
      el.setAttribute('aria-hidden', 'true')
      layer!.appendChild(el)
    }
    el.style.top = `${page.top}px`
