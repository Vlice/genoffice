...
import { generateTocFieldXml, type StyleInfo, type TocEntry } from '@genoffice/docx-engine'
import { styleIdForOutlineLevel } from '../editor/headings'
import { fontPickPatch } from '../font-list'
...
function buildTextMarkPatch(cmd: {
  ...
  for (const f of cmd.fields) {
    if (f === 'font') {
      Object.assign(markPatch, fontPickPatch(cmd.style.font ?? null))
...
function runSetHeadingLevel(
  ...
  for (const b of matched) {
    ...
    // overwrite the saved pStyle — copying Heading1 onto a Heading2 (or Normal)
    // would keep [data-style=Heading1] CSS after the outline change
    const attrs: Record<string, unknown> = {
      ...b.node.attrs,
      styleId: styleIdForOutlineLevel(styles, cmd.level),
      aiChanged: true,
    }
      ...
      if ('updateTextStyle' in command) {
        ...
        results.push(
          runSetHeadingLevel(
            tr,
            schema,
            command.setHeadingLevel,
            sel,
            editor.storage.listNumbering?.styles,
          ),
        )
