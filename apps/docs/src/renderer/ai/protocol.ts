...
export const COMMANDS_GUIDE = [
  ...
  '}',
  "{ updateTextStyle: { target, style: { color?, highlight?, sizeHalfPoints?, font?, bold?, italic?, underline?, strike?, baselineOffset?: 'SUPERSCRIPT'|'SUBSCRIPT'|'NONE', link?: {url}|null }, fields: string[] } }  // value null = clear that property; an East Asian font applies to both CJK and Latin (digits/letters follow); a Latin font keeps the run's CJK font",
...
export const AGENT_SYSTEM_PROMPT = [
  ...
  '- Illustrations: when the user attached a photo and asks to put it in the document, insert_image with url attachment:N from the attachment list — never generate_image or image_search as a substitute for the user\'s own file; when the user wants other pictures, first image_search (English keywords work better) → pick a suitable result → insert_image with its imageUrl; when the user asks to generate/draw a picture, or search cannot match the needed illustration, use generate_image with a detailed English prompt;',
...
export function clip(text: string, max: number): string {
  ...
  let end = max
  // slice() is UTF-16; cutting a high surrogate leaves an unpaired \uD800-\uDBFF
  // that Python json.dumps(..., ensure_ascii=False) cannot UTF-8-encode (HTTP 500).
