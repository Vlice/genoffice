...
export const AGENT_TOOLS: AgentToolDef[] = [
  ...
  },
  {
    name: 'image_search',
    description:
      'Search for images. Returns a list of image imageUrl entries; after picking one, insert it into the document with insert_image. Do not search for a substitute of a user-uploaded photo � insert those with insert_image url attachment:N.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'insert_image',
    description:
      'Insert an image into the document (at the cursor / end of document). url is either a direct http(s) imageUrl from image_search, or attachment:N from the attachment list for a user-uploaded photo. Prefer attachment:N when the user asked to put their attached image into the document.',
    inputSchema: {
      type: 'object',
      properties: {
        url: {
          type: 'string',
          description: 'direct http(s) image link, or attachment:N for a user-uploaded photo',
        },
  ...
  },
  {
    name: 'generate_image',
    description:
      'Generate an illustration with AI from a text prompt and insert it into the document (at the cursor / end of document). For illustration/diagram-style art that image_search cannot find, or when the user asks to generate/draw a picture. Do not use this to recreate a user-uploaded photo � insert that photo with insert_image url attachment:N. Requires Genspark login with cloud tools enabled.',
    inputSchema: {
      type: 'object',
      properties: {
        prompt: {
...
const ATTACHMENT_REF_RE = /^attachment:(\d+)$/i
...
function isImageAttachment(att: AttachmentMeta | undefined): att is AttachmentMeta {
...
async function executeAsyncTool(
  ...
  switch (call.name) {
    ...
    case 'insert_image': {
      ...
      const attIndex = parseAttachmentIndex(url)
      if (attIndex != null) {
        return insertImageFromAttachment(editor, attIndex, getAttachments?.() ?? [], maxW, signal)
      }
      if (!/^https?:\/\//.test(url)) return fail(t('aiSumInsertImage'), 'invalid url')
      return insertImageFromUrl(editor, url, maxW, signal, {
...
/** decode webp (and other browser-readable formats) to png so Word can embed them */
async function rasterToPng(
  base64: string,
  mime: string,
): Promise<{ base64: string; mime: 'image/png' } | null> {
  try {
    ...
    if (!m?.[1]) return null
    return { base64: m[1], mime: 'image/png' }
  } catch {
    return null
  }
}
...
async function embeddableImage(
  base64: string,
  reportedMime?: string,
): Promise<{ base64: string; mime: 'image/png' | 'image/jpeg' | 'image/gif' } | null> {
  const sniffed = sniffImageMime(base64)
  if (sniffed) return { base64, mime: sniffed }
  const converted = await rasterToPng(base64, reportedMime || 'image/webp')
  return converted
}

async function insertImageFromAttachment(
  editor: Editor,
  index: number,
  ...
  if (!isImageAttachment(att)) {
    return fail(
      t('aiSumInsertImage'),
      'invalid attachment index (see the attachment list; only png/jpg/gif/webp image attachments can be inserted)',
    )
  }
  const fetched = await window.desktop.readAttachmentImage(att.path)
  if (signal?.aborted)
    return fail(t('aiSumInsertImage'), 'stopped by the user; the image was not inserted')
  if (!fetched.ok || !fetched.base64) {
    return fail(t('aiSumInsertImage'), fetched.error ?? 'the attachment image could not be read')
  }
  const embed = await embeddableImage(fetched.base64, fetched.mime)
  if (!embed) {
    return fail(
      t('aiSumInsertImage'),
      'unsupported image format (only png/jpg/gif can be embedded; webp is converted when possible)',
    )
  }
  return insertImageFromBase64(editor, embed.base64, embed.mime, maxW, signal, {
    failLabel: t('aiSumInsertImage'),
    doneLabel: t('aiSumInsertImage'),
    blockLabel: `Image (${att.name})`,
  })
}
...
async function insertImageFromUrl(
  ...
  return insertImageFromBase64(editor, fetched.base64, mime, maxW, signal, labels)
}

async function insertImageFromBase64(
  editor: Editor,
  base64: string,
...
export function executeTool(
  ...
  if (
    ...
    return executeAsyncTool(editor, call, signal, getAttachments)
  }
