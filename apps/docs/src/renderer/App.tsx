...
import { PageThumbs, scrollDocsToPage } from './components/PageThumbs'
...
import {
  ...
  compareWithOpenResult as compareWithOpenResultImpl,
...
/** MoreAI cloud-docs iframe: always-on autosave; QAT shows status instead of Save / toggle. */
const MOREAI_EMBED = new URLSearchParams(window.location.search).get('moreai') === '1'

function openDocHyperlink(href: string): void {
  const raw = href.trim()
  if (!raw || raw.startsWith('#') || /^javascript:/i.test(raw)) return
  const url = /^[a-z][a-z0-9+.-]*:/i.test(raw) ? raw : `https://${raw}`
  if (!/^(https?:|mailto:|tel:)/i.test(url)) return
  const openExt = (
    window as Window & { desktop?: { openExternal?: (u: string) => void | Promise<void> } }
...
export function App() {
  ...
  /** KB embed: PDF-style page thumbnail rail (default open). */
  const [showPageThumbs, setShowPageThumbs] = useState(embedReadOnly)
  const [viewMode, setViewMode] = useState<ViewMode>('print')
  const [readMode, setReadMode] = useState(embedReadOnly)
  ...
  const [autoSave, setAutoSave] = useState(
    () => MOREAI_EMBED || localStorage.getItem('aidocs.autoSave') === '1',
  )
  useEffect(() => {
    if (MOREAI_EMBED) setAutoSave(true)
  }, [])
        ...
        setDoc((prev) =>
          prev && prev.filePath === oldPath
            ? {
                ...prev,
                filePath: newPath,
                fileName: fileNameFromPath(newPath, prev.fileName),
              }
            : prev,
        )
      }),
  ...
  const fieldValue = useCallback(
    (instr: string): string => {
      ...
      switch (kw) {
        ...
        case 'FILENAME':
          return fileNameFromPath(doc?.filePath, doc?.fileName ?? '') || doc?.fileName || ''
        default:
          return ''
      }
    },
  ...
  const compareWithOpenResult = useCallback(
    (file: Parameters<typeof compareWithOpenResultImpl>[1]) =>
      compareWithOpenResultImpl(reviewCtxRef.current, file),
    [],
  )
  ...
  useLayoutEffect(() => {
    if (readMode) {
      if (embedReadOnly) {
        // KB iframe embed: fit page width — 1:1 zoom overflows narrow panes.
        requestAnimationFrame(() => zoomFit('width'))
        return
      }
  ...
  useEffect(() => {
    ...
    w.__MOREAI_EMBED_ZOOM_FIT__ = () => zoomFit('width')
    return () => {
  ...
  useEffect(() => {
    if (!embedReadOnly || !doc) return
    requestAnimationFrame(() => zoomFit('width'))
  }, [doc, embedReadOnly, zoomFit])
  ...
  useEffect(() => {
    ...
    const remeasure = () => {
      ...
      if (editor) {
        ...
        // KB embed is readMode (no ribbon) but still needs print page-gaps so thumbs
        // can jump and the canvas shows PDF-like page separation.
        const showPrintPageGaps = viewMode === 'print' && (!readMode || embedReadOnly)
        if (showPrintPageGaps) {
          const pmRect = pm.getBoundingClientRect()
        ...
        syncCutOverlays((pm.closest('.page-wrap') as HTMLElement) ?? pm, overlayCutAnchors, factor)
        {
          ...
          syncPageBorders(wrapEl, borderStyle, factor)
          syncPageWatermarks(wrapEl, watermark, factor)
        }
        syncMarginAnnotations(
    ...
    return window.desktop.onMenuCommand((command, payload) => {
      ...
      switch (command) {
        ...
        case 'shortcuts':
          ...
          break
        case 'bold':
          if (editor) toggleStyleAwareMark(editor, 'bold')
          break
        case 'italic':
          if (editor) toggleStyleAwareMark(editor, 'italic')
          break
        case 'underline':
  ...
  const onDocClick = useCallback(
    (e: ReactMouseEvent) => {
      ...
      // the renderer in place. MoreAI web embed: a plain click opens the URL
      // (飞书/在线文档习惯). Native GenOffice keeps Word's ⌘/Ctrl+click.
      const a = (e.target as HTMLElement).closest('a[href]') as HTMLAnchorElement | null
      if (a) {
        e.preventDefault()
        const href = a.getAttribute('href') ?? ''
        if (MOREAI_EMBED || e.metaKey || e.ctrlKey) openDocHyperlink(href)
      }
  ...
  const quickActions = useMemo(
    () => (
      <>
        {MOREAI_EMBED ? (
          <span
            className={`autosave-status${hasUnsavedChanges ? ' dirty' : ''}`}
            role="status"
            aria-live="polite"
          >
            {hasUnsavedChanges ? '未保存' : '已自动保存'}
          </span>
        ) : (
          <button
            className="qa-btn"
...
.editor-scroll .doc-page.measuring-columns { column-count: auto; width: ${colFlow.colWidthPx + twipsToPx(canvasSection?.marginLeft ?? section?.marginLeft ?? 0) + twipsToPx(canvasSection?.marginRight ?? section?.marginRight ?? 0)}px; }`}</style>
      ...
      <div className="app-main">
        ...
        <div className="app-content">
          <div className={`workspace ${darkCanvas ? 'workspace-dark' : ''}${embedReadOnly ? ' workspace-embed' : ''}`}>
            {embedReadOnly && doc && (
              <button
                type="button"
                className={`doc-thumbs-toggle${showPageThumbs ? ' is-open' : ''}`}
                aria-label={showPageThumbs ? 'Hide page thumbnails' : 'Show page thumbnails'}
                title={showPageThumbs ? 'Hide page thumbnails' : 'Show page thumbnails'}
                onClick={() => {
                  setShowPageThumbs((v) => !v)
                  requestAnimationFrame(() => zoomFit('width'))
                  window.setTimeout(() => zoomFit('width'), 80)
                }}
              >
                <span className="doc-thumbs-toggle-icon" aria-hidden>
                  {showPageThumbs ? '‹' : '›'}
                </span>
              </button>
            )}
            {embedReadOnly && doc && showPageThumbs && section && (
              <PageThumbs
                current={pageInfo.current}
                total={pageInfo.total}
                pageWidthPx={twipsToPx(section.pageWidth)}
                pageHeightPx={twipsToPx(section.pageHeight)}
                revision={pageInfo.total * 1000 + Math.round(zoom)}
                onGoToPage={(page) => {
                  scrollDocsToPage(page)
                }}
              />
            )}
            {doc && showFind && (
        ...
        <PaginationPreview
          ...
          clearPageGaps={() => {
            ...
            if (wrap) {
              ...
              syncPageWatermarks(wrap as HTMLElement, null, 1)
      ...
      {showCompareDialog && (
        <CompareDialog
          excludeId={doc?.filePath}
          onCancel={() => setShowCompareDialog(false)}
          onPicked={(file) => {
            setShowCompareDialog(false)
            void compareWithOpenResult(file)
          }}
        />
      )}
