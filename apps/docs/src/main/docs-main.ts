...
export function registerDocsIpc(): void {
  ...
  ipcMain.handle('docs:compare-list', () => {
    const paths = readJson<string[]>(RECENT_PATH(), []).filter((p) => existsSync(p))
    return {
      source: 'recent' as const,
      items: paths.map((p) => ({ id: p, name: basename(p) })),
    }
  })

  ipcMain.handle('docs:compare-read', (event, id: string) => {
    if (typeof id !== 'string' || !id) return null
    return loadDocx(id, event.sender.id)
  })
