...
/** Review → Compare: a cloud/recent document the user can pick without replacing the open file */
export interface CompareDocCandidate {
  id: string
  name: string
}

export interface CompareDocList {
  source: 'cloud' | 'recent'
  items: CompareDocCandidate[]
}
...
export interface DesktopApi {
  ...
  /**
   * Review → Compare picker: cloud artifacts (MoreAI) or recent files (native).
   * Does not open/replace the current document.
   */
  listCompareDocs(): Promise<CompareDocList>
  /** Load bytes for a listCompareDocs id without replacing the current document. */
  readCompareDocx(id: string): Promise<OpenDocxResult>
  /** Native/local file picker for Compare (MoreAI must not reuse openDocx — that returns the current cloud file). */
  pickCompareDocx(): Promise<OpenDocxResult>
