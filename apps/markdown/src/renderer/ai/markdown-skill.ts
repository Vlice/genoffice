...
const AGENT_SYSTEM_PROMPT = [
  ...
  '- To regenerate or replace a SELECTED image ("用 AI 重新生成这张图", "换一张图", "regenerate this image"): call generate_image with replaceBlockIndex set to the selected image block index (from the user selection). That swaps the picture in place. Never insert-after then delete — indexes shift and you will often delete the NEW image instead of the old one.',
  '- If you already inserted a duplicate after block N by mistake: delete block N (the original), not N+1 (the new one). Prefer calling get_document_context and checking image previews (filename/alt) before deleting.',
  '- replace_image swaps an existing image block for a URL without shifting indexes. Use it when you already have a URL from generate_image/image_search.',
  '- Both insert/replace paths save the image file next to the document, which requires the document to have been saved at least once; if a tool reports there is no save location, ask the user to save the file first.',
  '',
