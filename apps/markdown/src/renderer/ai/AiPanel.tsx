...
export interface MarkdownAiDeps {
  /**
   * TipTap instance for UI (scope chip, selection listeners). Must not change
   * the markdown source/rich view — doing so on render makes the source-mode
   * toolbar button a no-op while this panel is mounted.
   */
  getEditor(): Editor | null
  /**
   * Flush source-mode text into TipTap and switch to rich view so tools can
   * edit. Call only from AI runs and citation jumps, never from render.
   */
  prepareEditorForAi(): Editor | null
...
export function AiPanel({
  ...
  if (!loopRef.current) {
    loopRef.current = new AgentLoop<DocSnapshot>({
      ...
      skill: composeSkills('markdown+search', '', [
        createMarkdownSkill(() => depsRef.current.prepareEditorForAi(), {
  ...
  const send = (text: string, displayText?: string): void => {
    ...
    depsRef.current.prepareEditorForAi()
  ...
  // Peek only — prepareEditorForAi here would undo source mode on every mount.
  useEffect(() => {
    const editor = depsRef.current.getEditor()
    if (!editor) return
    const bump = () => {
  ...
  // Peek on render: must not call prepareEditorForAi (that forces rich view).
  const editor = depsRef.current.getEditor()
  ...
  const docNav = {
    ...
    onNavigate: (href: string) => {
      ...
      const current = depsRef.current.prepareEditorForAi()
