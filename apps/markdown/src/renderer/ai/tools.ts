...
function blockPreview(node: PmNode): string {
  if (node.type.name === 'image') {
    ...
    const label = alt || file || '(image)'
    return label.length > PREVIEW_CHARS ? `${label.slice(0, PREVIEW_CHARS)}…` : label
  }
...
export const AGENT_TOOLS: AgentToolDef[] = [
  ...
  },
  {
    name: 'generate_image',
    description:
      'Generate an illustration with AI from a text prompt. By default inserts it after a top-level block. When the user selected an image and asks to regenerate/replace it, pass replaceBlockIndex (the selected image block) so the new image swaps in place — never insert-after then delete (that shifts indexes and often deletes the new image). Requires Genspark login with cloud tools enabled, and the document must have been saved to disk at least once.',
    inputSchema: {
      type: 'object',
      properties: {
        prompt: {
        ...
        afterIndex: {
          type: 'integer',
          description:
            '0-based block index to insert after; -1 = document start. Defaults to the end of the document. Ignored when replaceBlockIndex is set.',
        },
        replaceBlockIndex: {
          type: 'integer',
          description:
            '0-based index of an existing image block to replace in place (same position). Use this for "regenerate this image" / "replace this picture" when the user has selected an image. Prefer over insert+delete.',
        },
        alt: { type: 'string', description: 'Alt text describing the image' },
      },
      required: ['prompt'],
  ...
  },
  {
    name: 'replace_image',
    description:
      'Replace an existing image block in place with a URL (from generate_image or image_search). Same position; no index shift. Prefer this (or generate_image with replaceBlockIndex) when regenerating a selected image — never insert a duplicate then delete.',
    inputSchema: {
      type: 'object',
      properties: {
        blockIndex: {
          type: 'integer',
          description: '0-based index of the image block to replace',
        },
        url: { type: 'string', description: 'Direct image URL (http/https)' },
        alt: { type: 'string', description: 'Alt text describing the image' },
      },
      required: ['blockIndex', 'url'],
    },
  },
  {
...
/** download a direct image URL, persist it beside the document, insert or replace an image block */
async function insertImageFromUrl(
  editor: Editor,
  url: string,
  input: { afterIndex?: unknown; alt?: unknown; replaceBlockIndex?: unknown },
  ...
  // In-place replace: regenerate/swap selected image without shifting indexes.
  const replaceRaw = input.replaceBlockIndex
  if (typeof replaceRaw === 'number' && Number.isInteger(replaceRaw)) {
    const replaceIndex = clampIndex(replaceRaw, maxIndex)
    if (replaceIndex === null) {
      return fail(
        `replaceBlockIndex ${replaceRaw} is out of range (0–${maxIndex})`,
        labels.fail,
      )
    }
    const existing = doc.child(replaceIndex)
    if (existing.type.name !== 'image') {
      return fail(
        `Block ${replaceIndex} is not an image (type: ${existing.type.name})`,
        labels.fail,
      )
    }
    const { from, to } = blockRange(doc, replaceIndex, replaceIndex)
    let tr = editor.state.tr.replaceWith(from, to, node)
    tr = markAiRange(tr, from, from + node.nodeSize)
    editor.view.dispatch(tr)
    if (!userEditedDuringFetch) markDocSeen(editor)
    return {
      output: `Replaced the image at block ${replaceIndex} in place (saved as ${rel}). Block indexes are unchanged.`,
      mutated: true,
      summary: labels.done,
    }
  }
  ...
  if (isBlankDoc(doc)) {
    ...
    insertedIndex = after === -1 ? 0 : after + 1
  }
  editor.view.dispatch(tr)
  if (!userEditedDuringFetch) markDocSeen(editor)
  return {
    output: `Inserted the image at block index ${insertedIndex} (saved as ${rel}). If you meant to replace an existing image, use replaceBlockIndex / replace_image instead of deleting. ${INDEX_CHANGE_NOTICE}`,
    mutated: true,
    summary: labels.done,
  }
}
...
export function executeTool(
  ...
  switch (call.name) {
    ...
    case 'generate_image': {
      ...
      const replacing =
        typeof call.input.replaceBlockIndex === 'number' &&
        Number.isInteger(call.input.replaceBlockIndex)
      return window.markdownApi
        ...
        .then((generated): ToolExecution | Promise<ToolExecution> => {
          ...
          return insertImageFromUrl(editor, generated.url, call.input, signal, {
            ...
            done: replacing ? t('aiToolReplaceImageDone') : t('aiToolGenImageDone'),
          })
        })
    }

    case 'replace_image': {
      if (editedExternally(editor)) return fail(STALE_DOC_ERROR, t('aiToolReplaceImage'))
      const url = String(call.input.url ?? '')
      if (!/^https?:\/\//.test(url)) return fail('invalid url', t('aiToolReplaceImage'))
      return insertImageFromUrl(
        editor,
        url,
        { replaceBlockIndex: call.input.blockIndex, alt: call.input.alt },
        signal,
        {
          fail: t('aiToolReplaceImage'),
          done: t('aiToolReplaceImageDone'),
        },
      )
    }
