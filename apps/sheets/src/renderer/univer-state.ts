...
export interface LazyWorkbookState {
  ...
  /// Bumped on row/column insert or delete. In-flight viewport loads capture
  /// this and abort instead of evicting the post-shift grid with a stale
  /// range that is now out of bounds (that wipe is what made "delete columns"
  /// look like it cleared the whole sheet).
  streamEpoch: number
...
/// Drop in-flight / retrying range loads for a sheet whose grid just shifted.
export function invalidateSheetStream(state: LazyWorkbookState, sheetId: string): void {
  state.streamEpoch = (state.streamEpoch ?? 0) + 1
  const timer = state.retryTimers.get(sheetId)
