...
export async function loadVisibleRange(
  ...
  // Also cap to Univer's live grid — after delete_cols the model shrinks
  // immediately, and a file+delta request that still includes the old last
  // column throws "Range is out of bounds" and never reinstalls cells.
  const ops = state.editJournal.structuralOps.get(sheetId) ?? []
  const liveGrid = worksheetGridSize(worksheet)
  const screenRowCount = Math.min(
    Math.max(sheet.rowCount + netAxisDelta(ops, 'row'), 0),
    liveGrid?.rows ?? Number.POSITIVE_INFINITY,
  )
  const screenColumnCount = Math.min(
    Math.max(sheet.columnCount + netAxisDelta(ops, 'column'), 0),
    liveGrid?.columns ?? Number.POSITIVE_INFINITY,
  )
  ...
  for (let startRow = bounds.startRow; startRow <= bounds.endRow; startRow += chunkRows) {
    ...
    for (const cell of mapped?.screen.cells ?? []) {
      ...
      if (typeof value === 'string') {
        // Shared-string / inlineStr numbers ("1299") must become NUMBER or
        // Univer paints ForceString ("Number stored as text") and SUM skips them.
        const asNumber = textToNumber(value)
        if (asNumber !== undefined) {
          target.v = asNumber
          target.t = CellValueType.NUMBER
        } else {
          target.v = value
...
export async function readChartGridValues(
  ...
  const readUniverRaw = (): (string | number | boolean | null | undefined)[][] =>
    target.getRange(range).getRawValues() as (string | number | boolean | null | undefined)[][]
  ...
  // Mapped/sidecar holes (or MoreAI open races) can leave an all-empty matrix
  // while Univer already paints the selection — fall back so Insert Chart works.
  const hasAnyValue = grid.some((row) =>
    row.some((value) => value !== null && value !== undefined && value !== ''),
  )
  if (!hasAnyValue) {
    try {
      const raw = readUniverRaw()
      if (raw.some((row) => row.some((value) => value !== null && value !== undefined && value !== ''))) {
        return raw
      }
    } catch {
      /* keep mapped grid */
...
async function loadRange(
  ...
  try {
    ...
    const liveRange = clampRangeToWorksheet(worksheet, range)
    if (!liveRange) return
    const liveLoaded = loaded ? clampRangeToWorksheet(worksheet, loaded) : undefined
    if (!mapped) {
      ...
      state.loadedRanges.set(sheetId, liveRange)
      return
    }
    const availableEndRow =
      mapped.indexedThroughScreen === null
        ? null
        : Math.min(mapped.indexedThroughScreen, liveRange.endRow)
    let patchedRange: IRange | undefined
    if (availableEndRow !== null && availableEndRow >= liveRange.startRow) {
      const availableRange = { ...liveRange, endRow: availableEndRow }
      const alreadyLoadedRaw = state.loadedRanges.get(sheetId)
      const alreadyLoaded = alreadyLoadedRaw
        ? clampRangeToWorksheet(worksheet, alreadyLoadedRaw)
        : undefined
    ...
    const wrapWindow = patchedRange ?? (result.indexingComplete ? liveRange : undefined)
    if (wrapWindow && !bulk) {
    ...
    if (!result.indexingComplete) {
      ...
      if (waitForRequestedRange && nextStalls !== null) {
        ...
        if (lazyWorkbookRef.current === state && state.streamEpoch === epoch) {
        ...
        const timer = setTimeout(() => {
          if (lazyWorkbookRef.current !== state || state.streamEpoch !== epoch) return
          state.loadingKeys.delete(sheetId)
          void loadRange(runtime, lazyWorkbookRef, worksheet, liveRange, setMessage, true).then(
            () => loadFrozenColumnStrip(lazyWorkbookRef, worksheet, sheet, liveRange),
          )
        }, 250)
...
export function clampRangeToGrid(
  range: IRange,
  rowCount: number,
  columnCount: number,
): IRange | null {
  ...
  const endRow = Math.min(range.endRow, rowCount - 1)
  const endColumn = Math.min(range.endColumn, columnCount - 1)
  if (startRow > endRow || startColumn > endColumn) return null
  return { startRow, startColumn, endRow, endColumn }
}

function clampRangeToWorksheet(
  worksheet: object,
  range: IRange,
): IRange | null {
  const grid = worksheetGridSize(worksheet)
  return grid ? clampRangeToGrid(range, grid.rows, grid.columns) : range
}

function worksheetGridSize(
  worksheet: object,
): { rows: number; columns: number } | null {
  const rows = (worksheet as { getMaxRows?: () => number }).getMaxRows?.()
  const columns = (worksheet as { getMaxColumns?: () => number }).getMaxColumns?.()
        ...
        [
          cell.f !== undefined
            ...
            : typeof cell.v === 'string' && cell.v !== ''
              ? (() => {
                  const asNumber = textToNumber(cell.v)
                  return asNumber !== undefined
                    ? { v: asNumber, t: CellValueType.NUMBER }
...
export function patchWorksheetRangeInner(
  ...
  const previous = grid
    ? previousRange
      ? clampRangeToGrid(previousRange, grid.rows, grid.columns)
      : undefined
    : previousRange
  const target = grid ? clampRangeToGrid(range, grid.rows, grid.columns) : range
  if (!target) return
  range = target
  previousRange = previous ?? undefined
  // A follow-up read that returns no values must not erase cells already
  // painted for this window. That re-read is what makes imported data flash
  // and then vanish. Scrolling to a different window still evicts.
  const incomingHasContent = cells.some(
    (cell) => (cell.value != null && cell.value !== '') || Boolean(cell.formula),
  )
  const coversPrevious =
    previousRange != null &&
    range.startRow <= previousRange.startRow &&
    range.endRow >= previousRange.endRow &&
    range.startColumn <= previousRange.startColumn &&
    range.endColumn >= previousRange.endColumn
  if (previousRange && !(coversPrevious && !incomingHasContent)) {
            ...
            : typeof displayValue === 'string' && joinLines
              ...
              : typeof displayValue === 'string' && multiline
                ...
                : typeof displayValue === 'string' && displayValue !== ''
                  ? (() => {
                      // Explicit string typing protects codes ("007"); coerce
                      // plain amounts so ForceString / SUM treat them as numbers.
                      const asNumber = textToNumber(displayValue)
                      return asNumber !== undefined
                        ? { v: asNumber, t: CellValueType.NUMBER }
...
export function queueVisualInstall(
  ...
  visualInstallTimerRef.current = setTimeout(function install() {
    ...
    // A dropped pointerup (iframe leave) can leave the drag flag stuck and
    // the host hidden. After 10s, force-clear so the replacement can mount.
