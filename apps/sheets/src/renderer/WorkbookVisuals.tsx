...
import { relaxFloatDomScrollClamp } from './float-dom-scroll-fix'
...
/// Session-inserted pictures (Insert → Picture / screenshot / AI) use the
/// same journal path as shapes — without this they render read-only and
/// Delete/Backspace never hooks up.
export function isEditableSessionImage(visual: WorkbookVisualObject): boolean {
...
export function installWorkbookVisuals(
  ...
  for (const visual of file.visuals.filter((candidate) => candidate.sheetId === sheetId)) {
    ...
    const floatId = `${file.sessionId}-${visual.id}`
    ...
    // Charts/shapes must scroll with cells; Univer's default viewport clamp
    // pins/shrinks them and kills hit-testing once scrolled. Relax even when
    // addFloatDomToRange early-returns on a leftover rect (same float id).
    relaxFloatDomScrollClamp(runtime, floatId)
    requestAnimationFrame(() => relaxFloatDomScrollClamp(runtime, floatId))
  }
  return disposables
}
...
export function restoreHostVisual(host: HTMLElement | null | undefined): void {
  if (!host) return
...
/// A dropped pointerup (MoreAI iframe leave without lostpointercapture) can
/// leave the count stuck and the host hidden. Callers that must remount wait
/// a while, then force-clear so the chart becomes hittable again.
export function unstickVisualDragIfStale(maxAgeMs = 10_000): boolean {
  if (activeDragCount <= 0) return false
  if (dragStartedAt !== 0 && Date.now() - dragStartedAt <= maxAgeMs) return false
  activeDragCount = 0
  dragStartedAt = 0
  discardDropGhost()
  return true
...
function EditableShapeVisual({
  ...
  const setDragActive = (active: boolean): void => {
    ...
    if (active) {
      if (activeDragCount === 0) dragStartedAt = Date.now()
      activeDragCount += 1
    } else if (activeDragCount > 0) {
      activeDragCount -= 1
      if (activeDragCount === 0) dragStartedAt = 0
    }
  }
  ...
  const finishDrag = (commit: boolean): void => {
    ...
    setDragActive(false)
    if (!session && !wasActive && !ghostRef.current) return
    const host = session?.host ?? hostNodeRef.current
    if (session) {
      try {
        session.host.releasePointerCapture(session.pointerId)
      } catch {
    ...
    const committed =
      ...
      commitDrag(session.mode, session.corner, dx, dy)
    if (committed && ghostRef.current && host) {
      parkDropGhost(visual.id, ghostRef.current, host)
      ghostRef.current = null
    } else {
      removeGhost(host)
    }
  ...
  return (
    <div
      ...
      onPointerDown={(event) => {
        ...
        const session = {
          mode,
          corner: handleCorner ?? ('se' as const),
        ...
        const onMove = (moveEvent: PointerEvent): void => {
          ...
          if (!ghostRef.current && (Math.abs(dx) > 2 || Math.abs(dy) > 2)) {
            ...
            // opacity:0 keeps pointer capture; visibility:hidden often drops it
            // (MoreAI iframe) and leaves a non-interactive ghost on top.
        ...
        const onLost = (lostEvent: PointerEvent): void => {
          if (lostEvent.pointerId !== pointerId) return
          // Iframe leave / capture drop without pointerup: commit last delta
          // so the chart stays where the ghost is, then park/claim restores
          // the host. Cancelling here left the host hidden and unselectable.
          finishDragRef.current(true)
        }
        const onBlur = (): void => {
...
function BarChart({
  ...
  const angledCats = categoryAxisAngled(categories, visibleCount, columnWidth)
  const tickStride = angledCats ? 1 : categoryTickStride(categories, visibleCount, columnWidth)
...
/// CJK and other wide glyphs take about two Latin advances. Counting them as
/// one character lets 笔记本电脑 overlap its neighbors in a column chart.
function axisLabelUnits(text: string): number {
  let units = 0
...
export function categoryTickLines(label: string, slotWidth: number): readonly string[] {
  const fits = (text: string): boolean => axisLabelUnits(text) * AXIS_LABEL_CHAR_UNITS <= slotWidth
...
/// Excel rotates the whole category axis together. One overflowing label
/// (笔记本电脑 next to 显示器) must not leave the shorter ones horizontal.
export function categoryAxisAngled(
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    if (axisLabelUnits(label) * AXIS_LABEL_CHAR_UNITS > slotWidth) return true
  }
  return false
}
...
export function categoryTickStride(
  ...
  for (let index = 0; index < count; index += 1) {
    const label = labels[index] ?? String(index + 1)
    // Wide glyphs are drawn at an angle (CategoryTick) instead of thinned.
    if (axisLabelUnits(label) > label.length) continue
    for (const line of categoryTickLines(label, slotWidth)) {
      widest = Math.max(widest, Math.min(axisLabelUnits(line), 16) * AXIS_LABEL_CHAR_UNITS)
    }
  }
...
function CategoryTick({
  ...
  const budget = Math.max(5, Math.min(16, Math.floor(slot / AXIS_LABEL_CHAR_UNITS)))
  const lines = categoryTickLines(label, slot)
