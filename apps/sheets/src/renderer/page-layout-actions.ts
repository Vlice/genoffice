...
const isMoreaiEmbed = (): boolean =>
  typeof window !== 'undefined' &&
  new URLSearchParams(window.location.search).get('moreai') === '1'

/** Status line after a Page Layout journal write (MoreAI has no Save button). */
function pageSetupRecordedMessage(note: string): string {
  if (!isMoreaiEmbed()) return t('appPageSetupRecorded', { note })
  return getLang() === 'zh'
    ? `${note}——正在自动保存（打印相关设置不会改动网格显示）。`
    : `${note} — autosaving (print layout does not change the grid view).`
}

function themeAppliedMessage(name: string): string {
  if (!isMoreaiEmbed()) return t('appThemeApplied', { name })
  return getLang() === 'zh'
    ? `已应用主题“${name}”——正在自动保存。`
    : `Theme "${name}" applied — autosaving.`
}
...
export interface PageLayoutContext {
  ...
  /// MoreAI: flush journal to host immediately (print settings never paint the grid).
...
export function handlePageLayoutCommand(ctx: PageLayoutContext, rest: string): void {
  ...
  const record = (patch: PageSetupJournalState, note: string): void => {
    ...
    ctx.setMessage(pageSetupRecordedMessage(note))
    if (isMoreaiEmbed()) showToast(pageSetupRecordedMessage(note))
  ...
  switch (key) {
    ...
    case 'theme-fonts': {
      ...
      const themeName = (colors ?? fonts)?.name ?? value
      ctx.setMessage(themeAppliedMessage(themeName))
      if (isMoreaiEmbed()) showToast(themeAppliedMessage(themeName))
      ctx.flushPendingSave?.()
      return
    }
    case 'breaks': {
...
export function handleApplyHeaderFooter(
  ...
  if (isMoreaiEmbed()) {
    ctx.setMessage(
      getLang() === 'zh' ? '页眉页脚已更新——正在自动保存。' : 'Header & footer updated — autosaving.',
    )
  } else {
