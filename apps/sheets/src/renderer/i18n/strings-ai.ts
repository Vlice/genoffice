...
export const aiStrings = defineStrings({
  zh: {
    ...
    aiCheckBtn: 'AI 校验',
    aiCheckPrompt:
      '检查这个表格的问题（只读审计：先 get_workbook_context / find_cells / read_range 各读一次所需数据，然后立刻用纯文字回复，禁止再次调用只读工具）:1) 公式错误(#REF!、#DIV/0! 等)、引用范围没覆盖全部数据、合计行漏加列;2) 同一列数字和文本混用、日期格式不统一、明显的异常值和重复行;3) 合计与明细对不上、百分比加总不是 100% 等结构问题。列出发现的问题并给出修复建议；若未发现问题也请明确说明。不要调用 propose_operations。',
    aiAnalyzeBtn: 'AI 分析',
    aiAnalyzePrompt:
      '分析这个表格的数据（只读分析：先读取一次所需数据，然后立刻用纯文字总结，禁止再次调用同一批只读工具），总结关键发现、趋势和异常。不要调用 propose_operations。',
  ...
  en: {
    ...
    aiCheckBtn: 'AI Check',
    aiCheckPrompt:
      'Check this sheet for problems (read-only audit: call get_workbook_context / find_cells / read_range once for what you need, then answer immediately in plain text — do not re-call the same read-only tools): 1) formula errors (#REF!, #DIV/0!, etc.), ranges that do not cover all the data, totals rows missing columns; 2) numbers and text mixed in one column, inconsistent date formats, obvious outliers and duplicate rows; 3) totals that do not match the detail rows, percentage columns that do not add up to 100%. List the issues found and suggest fixes; if nothing is wrong, say so clearly. Do not call propose_operations.',
    aiAnalyzeBtn: 'AI Analyze',
    aiAnalyzePrompt:
      'Analyze the data in this sheet (read-only: read the needed data once, then summarize immediately in plain text — do not re-call the same read-only tools) and summarize key findings, trends, and anomalies. Do not call propose_operations.',
  ...
  'zh-TW': {
    ...
    aiCheckBtn: 'AI 校驗',
    aiCheckPrompt:
      '檢查這個表格的問題（唯讀審計：先 get_workbook_context / find_cells / read_range 各讀一次所需資料，然後立刻用純文字回覆，禁止再次呼叫唯讀工具）:1) 公式錯誤(#REF!、#DIV/0! 等)、參照範圍沒涵蓋全部資料、合計列漏加欄位;2) 同一欄數字和文字混用、日期格式不一致、明顯的異常值和重複列;3) 合計與明細對不上、百分比加總不是 100% 等結構問題。列出發現的問題並提出修復建議；若未發現問題也請明確說明。不要呼叫 propose_operations。',
    aiAnalyzeBtn: 'AI 分析',
    aiAnalyzePrompt:
      '分析這個表格的資料（唯讀分析：先讀取一次所需資料，然後立刻用純文字總結，禁止再次呼叫同一批唯讀工具），總結關鍵發現、趨勢和異常。不要呼叫 propose_operations。',
    aiSettingsTitle: 'AI 設定',
    aiSetUp: '設定 AI',
    aiNewChat: '新對話',
