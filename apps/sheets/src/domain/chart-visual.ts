...
/// Accept plain numbers and common display strings ("1,234.56", "¥100", "12%").
/// Reject labels like "ORD-2025" that happen to contain digits.
export function coerceChartNumber(v: unknown): number | null {
...
const isNumeric = (v: unknown): boolean => coerceChartNumber(v) !== null
const toNumber = (v: unknown): number => coerceChartNumber(v) ?? 0
...
export function chartDataFromValues(
  ...
  // Short-and-wide → series-in-rows for Excel cross-tabs (blank corner) and
  // corpus shapes whose first row is numeric after the label cell. Vertical
  // tables with text headers in row 1 (订单编号 / 客户 / 单价…) stay
  // column-oriented even when rows ≤ columns — otherwise Insert Chart no-ops.
  const shortWide = sourceFirstRow.length > 1 && source.length <= sourceFirstRow.length
  const firstRowRestNumeric =
    sourceFirstRow.length > 1 &&
    sourceFirstRow.slice(1).every((value) => isBlank(value) || isNumeric(value)) &&
    sourceFirstRow.slice(1).some((value) => isNumeric(value))
  const byRow = shortWide && (isBlank(sourceFirstRow[0]) || firstRowRestNumeric)
  const grid = byRow
...
/// Default float anchor for a freshly inserted chart. Wide selections place
/// the chart *under* the table so it stays on-screen in embed / narrow
/// viewports (endColumn+2 was easy to miss beside A1:J11 with DevTools open).
export function defaultChartAnchor(bounds: CellBounds): SheetVisualAnchor {
  const width = bounds.endColumn - bounds.startColumn + 1
  if (width >= 5) {
    return {
      fromRow: bounds.endRow + 2,
      fromColumn: bounds.startColumn,
      fromRowOffset: 0,
      fromColumnOffset: 0,
      toRow: bounds.endRow + 2 + DEFAULT_CHART_ROWS,
      toColumn: bounds.startColumn + DEFAULT_CHART_COLS - 1,
      toRowOffset: 0,
      toColumnOffset: 0,
    }
  }
  return {
    ...
    fromRowOffset: 0,
    fromColumnOffset: 0,
    toRow: bounds.startRow + DEFAULT_CHART_ROWS,
    toColumn: bounds.endColumn + 2 + DEFAULT_CHART_COLS - 1,
    toRowOffset: 0,
    toColumnOffset: 0,
  }
}
...
/// Insert Chart. Wide ranges anchor below the table; narrow ones stay to the right.
export function buildChartVisual(input: BuildChartVisualInput): SheetVisual {
  ...
  const anchor: SheetVisualAnchor =
    anchorBase === null
      ? defaultChartAnchor(bounds)
      : {
          fromRow: anchorBase.row,
          fromColumn: anchorBase.column,
          fromRowOffset: 0,
          fromColumnOffset: 0,
          toRow: anchorBase.row + DEFAULT_CHART_ROWS,
          toColumn: anchorBase.column + DEFAULT_CHART_COLS - 1,
          toRowOffset: 0,
          toColumnOffset: 0,
        }
