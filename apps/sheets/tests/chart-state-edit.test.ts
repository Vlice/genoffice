...
describe('chartDataFromValues orientation and header detection', () => {
  ...
  it('keeps vertical order tables column-oriented when rows ≤ columns', async () => {
    const { chartDataFromValues } = await import('../src/domain/chart-visual')
    const parsed = chartDataFromValues([
      ['订单编号', '客户', '单价', '数量', '金额'],
      ['ORD-1', '张三', 100, 2, 200],
    ...
    // 5 rows × 5 cols would flip under the old short-wide heuristic.
    expect(parsed?.byRow).toBe(false)
    expect(parsed?.hasCategoryColumn).toBe(true)
    expect(parsed?.series.map((s) => s.name)).toEqual(['单价', '数量', '金额'])
    expect(parsed?.categories).toEqual(['ORD-1', 'ORD-2', 'ORD-3', 'ORD-4'])
  })

  it('parses currency / thousand-separator display strings as numbers', async () => {
    const { chartDataFromValues, coerceChartNumber } = await import('../src/domain/chart-visual')
    expect(coerceChartNumber('¥1,234.5')).toBe(1234.5)
    expect(coerceChartNumber('ORD-2025')).toBeNull()
    const parsed = chartDataFromValues([
      ['Item', 'Amount'],
      ['A', '1,000'],
      ['B', '¥200'],
    ])
    expect(parsed?.series[0]?.values).toEqual([1000, 200])
  })

  it('places wide-selection charts below the table', async () => {
    const { defaultChartAnchor } = await import('../src/domain/chart-visual')
    expect(defaultChartAnchor({ startRow: 0, endRow: 10, startColumn: 0, endColumn: 9 })).toEqual({
      fromRow: 12,
      fromColumn: 0,
      fromRowOffset: 0,
      fromColumnOffset: 0,
