...
function extractTextboxes(
  ...
  const buildWpsBox = (
    ...
    const cNvPr = attrsOf(findChild(shape, 'wps:cNvPr') ?? {})
    const shapeId = cNvPr['id']
    if (!nested && shapeId) box.shapeId = shapeId
    // Gallery preset written by buildWordArtParagraphXml. The run color in
    // the file is only the solid fallback; the id is what the editor paints.
    const presetId = /^WordArt:([A-Za-z0-9-]+)$/.exec(cNvPr['name'] ?? '')?.[1]
    if (presetId) {
      box.wordArtId = presetId
      box.nowrap = true
    }
    if (spPr) {
