...
describe('per-data-point chart colors', () => {
  ...
  it('pie colorScheme maps onto per-slice pointColors (Change Colors gallery)', async () => {
    const opened = await openPptx(fx('01_standard_business.pptx'))
    const r = addChart(opened, 0, {
      kind: 'pie',
      categories: ['A', 'B', 'C', 'D'],
    ...
    const scheme = ['#111111', '#222222', '#333333', '#444444']
    expect(editChartElement(opened, 0, r.elementId, { colorScheme: scheme })).toBe(true)

    let reopened = await openPptx(await savePptx(opened))
    let el = reopened.deck.slides[0]!.elements.at(-1) as ChartElement
    expect(el.chart.series[0]!.pointColors).toEqual(scheme)

    // Switching schemes replaces slice colors (does not keep the previous dPt set)
    const next = ['#AA0000', '#00AA00', '#0000AA', '#AAAA00']
    expect(editChartElement(reopened, 0, el.id, { colorScheme: next })).toBe(true)
    reopened = await openPptx(await savePptx(reopened))
    el = reopened.deck.slides[0]!.elements.at(-1) as ChartElement
    expect(el.chart.series[0]!.pointColors).toEqual(next)
  })
})
