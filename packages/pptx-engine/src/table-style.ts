...
export function resolveTableStyle(
  ...
  // Built-in gallery IDs stay styled even after ppt/tableStyles.xml gains custom
  // <a:tblStyle> entries. The gallery injects a preset into that part when one table
  // is restyled; sibling and later-inserted tables still reference Medium Style 2
  // (or another builtin GUID) and must keep their look. Unknown ids stay unstyled.
