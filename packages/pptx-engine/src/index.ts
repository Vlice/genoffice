 ...
 */
import { JSZip, PackageArchive, relsPathFor, resolveTarget, type Relationship } from './zip'
...
export async function openPptx(bytes: Uint8Array): Promise<OpenedPptx> {
  ...
  // MoreAI's historical blank.pptx omitted required p:sldMasterIdLst; GenOffice
  // opens it, but WPS/PowerPoint refuse. Heal on open so the next save is valid.
  healPresentationMasterList(archive)
...
/**
 * Ensure presentation.xml declares every slideMaster relationship in
 * p:sldMasterIdLst (ECMA-376). Missing list is the usual reason WPS rejects
 * decks that GenOffice still opens.
 */
export function healPresentationMasterList(archive: PackageArchive): boolean {
  const PRES = 'ppt/presentation.xml'
  const xml = archive.readText(PRES)
  if (!xml) return false
  if (/<p:sldMasterIdLst[\s>]/.test(xml) || /<sldMasterIdLst[\s>]/.test(xml)) return false

  const rels = archive.readRels(PRES)
  const masterRels = [...rels.values()].filter((r) => r.type.endsWith('/slideMaster'))
  if (!masterRels.length) return false

  const list =
    '<p:sldMasterIdLst>' +
    masterRels
      .map((r, i) => `<p:sldMasterId id="${2147483648 + i}" r:id="${r.id}"/>`)
      .join('') +
    '</p:sldMasterIdLst>'

  let next = xml
  if (/<p:sldIdLst[\s>]/.test(next)) {
    next = next.replace(/<p:sldIdLst[\s>]/, `${list}$&`)
  } else if (/<sldIdLst[\s>]/.test(next)) {
    next = next.replace(/<sldIdLst[\s>]/, `${list}$&`)
  } else if (/<p:sldSz[\s/>]/.test(next)) {
    next = next.replace(/<p:sldSz[\s/>]/, `${list}$&`)
  } else {
    next = next.replace(/<\/p:presentation>/, `${list}</p:presentation>`)
  }

  if (!/<p:notesSz[\s/>]/.test(next) && !/<notesSz[\s/>]/.test(next)) {
    if (/<p:sldSz[^/]*\/>/.test(next)) {
      next = next.replace(/(<p:sldSz[^/]*\/>)/, '$1<p:notesSz cx="6858000" cy="9144000"/>')
    } else if (/<\/p:sldSz>/.test(next)) {
      next = next.replace(/<\/p:sldSz>/, '</p:sldSz><p:notesSz cx="6858000" cy="9144000"/>')
    }
  }

  if (next === xml) return false
  archive.entries.set(PRES, Buffer.from(next, 'utf8'))
  return true
}
...
function buildZip(opened: OpenedPptx): InstanceType<typeof JSZip> {
  const { deck, archive } = opened
...
export function editChartElement(
  ...
  // Pie/doughnut/pie3D paint slices from per-point colors (varyColors=1), not the
  // single series fill. Map "Change Colors" schemes onto category pointColors so
  // the gallery actually recolors wedges (bar/line keep series-level schemes).
  const pieLike = kind === 'pie' || kind === 'pie3D' || kind === 'doughnut'
  if (patch.colorScheme?.length && pieLike) {
    const scheme = patch.colorScheme
    const n = Math.max(categories.length, ...series.map((s) => s.values.length), 1)
    pointColors = series.map(() =>
      Array.from({ length: n }, (_, pi) => scheme[pi % scheme.length]!),
    )
  }
  ...
  // Series-level scheme injection is for multi-series charts; pie wedges already
  // carry colors via pointColors / <c:dPt>.
  const newXml = buildChartSpaceXmlWithColors(opts, pieLike ? undefined : colorScheme)
...
export interface ElementFontPatch {
  ...
  color?: string
  /** Super/subscript baseline % (positive = super, negative = sub, 0 = none) */
  baseline?: number
}
  ...
  for (const p of paragraphs) {
    ...
    for (const r of p.runs) {
      ...
      if (patch.baseline !== undefined) {
        if (!patch.baseline) delete r.baseline
        else r.baseline = patch.baseline
      }
    }
  }
}
